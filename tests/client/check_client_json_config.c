...
static void setup(void) {
    ...
    /* Allow longer timeouts so valgrind runs through */
    config->maxSessionTimeout = 100000;
    config->asyncOperationTimeout = 100000;
    ...
    ck_assert(!UA_ByteString_equal(&serverCertificate, &UA_BYTESTRING_NULL));
    ck_assert(!UA_ByteString_equal(&serverPrivateKey, &UA_BYTESTRING_NULL));
    ...
    UA_ByteString_clear(&serverPrivateKey);
#endif
    ...
    logins[0].username = UA_STRING_ALLOC(testUserName);
    logins[0].password = UA_STRING_ALLOC(testPassword);

    /* Configure AccessControl with username/password */
    retval = UA_AccessControl_default(config, true, &policy,
                                      sizeof(logins) / sizeof(logins[0]), logins);
    ck_assert(retval == UA_STATUSCODE_GOOD);
    ...
    ck_assert(retval == UA_STATUSCODE_GOOD);
#ifdef UA_ENABLE_ENCRYPTION
    retval = UA_ServerConfig_addEndpoint(config,
                                         (UA_String const) UA_STRING_STATIC("http://opcfoundation.org/UA/SecurityPolicy#Basic256Sha256"),
    ...
    if(config->endpointsSize > 0) {
        ...
        /* Allocate space for 2 token policies */
        endpoint->userIdentityTokens = (UA_UserTokenPolicy *)
            UA_Array_new(2, &UA_TYPES[UA_TYPES_USERTOKENPOLICY]);
        ...
        anonPolicy->policyId = UA_STRING_ALLOC("anonymous-policy#None");
        ...
        userPolicy->policyId = UA_STRING_ALLOC("username-policy#None");
        ...
        UA_UserTokenPolicy *certPolicy = &endpoint->userIdentityTokens[0];
        certPolicy->tokenType = UA_USERTOKENTYPE_CERTIFICATE;
        certPolicy->policyId = UA_STRING_ALLOC("certificate-policy#Basic256Sha256");
        certPolicy->securityPolicyUri = UA_STRING_ALLOC("http://opcfoundation.org/UA/SecurityPolicy#Basic256Sha256");
#endif
    }

#ifdef UA_ENABLE_ENCRYPTION
    /* Load client certificate for certificate-based authentication */
    UA_ByteString clientCertificate = loadFile("client_cert.der");
    ck_assert(!UA_ByteString_equal(&clientCertificate, &UA_BYTESTRING_NULL));

    UA_TrustListDataType trustList;
    UA_TrustListDataType_init(&trustList);

    /* Add client certificate to trusted list */
    ...
    retval = UA_CertificateGroup_Memorystore(&config->sessionPKI, &defaultUserTokenGroup,
...
#endif
    UA_String_clear(&logins[0].username);
    UA_String_clear(&logins[0].password);
...
START_TEST(loadClientConfig) {
    ...
    /* Check fields */
    /* test timeout field */
    ck_assert_uint_eq(clientConfig.timeout, 100000);
    ...
    const UA_String expected_anonPolicy_id = UA_STRING_STATIC("anonymous-policy#None");
    ...
    const UA_String expected_userPolicy_id = UA_STRING_STATIC("username-policy#None");
    ...
    const UA_String expected_certPolicy_id = UA_STRING_STATIC("certificate-policy#None");
...
#ifdef UA_ENABLE_ENCRYPTION
static char const * const file_name_certificate = "client_json_config_certificate.json5";

START_TEST(loadClientFromCertificateConfigAndConnectToServer) {
    UA_ByteString jsonConfig = loadFile(file_name_certificate);
...
#ifdef UA_ENABLE_ENCRYPTION
    tcase_add_test(tc_both, loadClientFromCertificateConfigAndConnectToServer);
#endif
