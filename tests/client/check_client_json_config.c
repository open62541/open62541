...
static void setup(void) {
    ...
    */
    ...
    if(config->endpointsSize > 0) {
        ...
        certPolicy->policyId = UA_STRING_ALLOC("certificate-policy#None");
    ...
    trustList.trustedCertificates = &clientCertificate;  // UA_ByteString
...
START_TEST(loadClientFromCertificateConfigAndConnectToServer) {
    ...
    /* Set the certificate and private key credentials on the client config.
     * The JSON config sets the userTokenPolicy to Certificate type,
     * but the actual credentials must be set via the API. */
    UA_ByteString certificate = loadFile("client_cert.der");
    UA_ByteString privateKey = loadFile("client_key.der");

    ck_assert(!UA_ByteString_equal(&certificate, &UA_BYTESTRING_NULL));
    ck_assert(!UA_ByteString_equal(&privateKey, &UA_BYTESTRING_NULL));
