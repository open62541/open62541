...
/* Note: We tried to use a server thread, instead of manually triggering UA_Server_run_iterate(), 
but we think that the ua_timer implementation would need a lock, because the unit test 
asnychronously adds and removes callbacks from a running server.
Even with a lock implementation we encountered segfaults, that's why we abandoned the 
multithreading test */
...
START_TEST(Test_many_components) {
    ...
    /*  Writers            : Interval  -> Readers            : Timeout
        ----------------------------------------------------------------------------------
        Conn 1 WG 1 - DSW1 : 500       -> Conn 2 RG 1 - DSR1 : 750
        Conn 1 WG 1 - DSW2 : 500       -> Conn 2 RG 2 - DSR2 : 850
        Conn 2 WG 1 - DSW1 : 250       -> Conn 1 RG 1 - DSR1 : 350
        Conn 2 WG 2 - DSW2 : 100       -> Conn 3 RG 1 - DSR1 : 200
    */
    ...
    UA_Duration MessageReceiveTimeout_Conn3_RG1_DSR1 = 200.0;
    ...
    const UA_UInt32 SleepTime = 20;
    const UA_UInt32 NoOfRunIterateCycles = 60;
    ...
    ValidatePublishSubscribe(VarId_Conn1_WG1_DS1, VarId_Conn2_RG1_DSR1, 33, SleepTime, NoOfRunIterateCycles);
    ...
    ValidatePublishSubscribe(VarId_Conn1_WG1_DS2, VarId_Conn2_RG2_DSR1, 100, SleepTime, NoOfRunIterateCycles);
    ...
    ValidatePublishSubscribe(VarId_Conn2_WG1_DS1, VarId_Conn1_RG1_DSR1, 50, SleepTime, NoOfRunIterateCycles);
    ...
    ValidatePublishSubscribe(VarId_Conn2_WG2_DS1, VarId_Conn3_RG1_DSR1, 124, SleepTime, NoOfRunIterateCycles);
    ...
    ValidatePublishSubscribe(VarId_Conn1_WG1_DS2, VarId_Conn2_RG2_DSR1, 48, SleepTime, NoOfRunIterateCycles);
