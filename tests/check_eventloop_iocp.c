...
START_TEST(tcpRepeatedCloseWithPendingReceive) {
    serverPort = 0;
    clientEstablished = 0;
    serverAccepted = 0;
    clientClosing = 0;
    ...
    el = UA_EventLoop_new_WIN32(NULL);
    cm = UA_ConnectionManager_new_WIN32_TCP(
        UA_STRING("iocp tcp repeated close"));
    ck_assert_ptr_ne(el, NULL);
    ck_assert_ptr_ne(cm, NULL);
    configureSmallBuffers();
    ck_assert_uint_eq(
    ...
    /* Every established client owns a pending WSARecv. Repeatedly cancel it,
     * drain its completion and reuse socket handles. */
    for(size_t i = 0; i < 64; i++) {
        ...
        ck_assert_uint_eq(
            cm->closeConnection(cm, clientId), UA_STATUSCODE_GOOD);
        for(size_t iteration = 0;
            iteration < 1000 && clientClosing < target; iteration++)
            ck_assert_uint_eq(el->run(el, 10), UA_STATUSCODE_GOOD);
        ck_assert_uint_eq(clientClosing, target);
    }
...
main(void) {
    ...
    tcase_add_test(testCase, tcpRepeatedCloseWithPendingReceive);
