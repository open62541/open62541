...
runWithShutdownCallback(UA_EventLoop *el, UA_UInt32 timeout) {
    ++probe.iterations;
    UA_Boolean targetIteration = probe.stopDriver ?
    ...
    if(probe.stopDriver) {
        ...
        probe.driver->state = UA_LIFECYCLESTATE_STOPPED;
...
static void
checkShutdown(UA_Boolean stopDriver) {
    memset(&probe, 0, sizeof(probe));
    probe.stopDriver = stopDriver;
    ...
    if(stopDriver) {
        probe.driver = (UA_Driver*)UA_calloc(1, sizeof(UA_Driver));
        ck_assert_ptr_ne(probe.driver, NULL);
        probe.driver->start = driverStart;
        probe.driver->stop = driverStop;
        probe.driver->free = driverFree;
        ck_assert_uint_eq(UA_Server_addDriver(probe.server, probe.driver),
                          UA_STATUSCODE_GOOD);
    } else {
...
START_TEST(shutdownDriversWithoutServerLock) {
    checkShutdown(true);
} END_TEST
...
int main(void) {
    ...
    tcase_add_test(tc, shutdownDriversWithoutServerLock);
