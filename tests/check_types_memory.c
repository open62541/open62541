...
START_TEST(encodeShallYieldDecode) {
    ...
    ck_assert_int_eq(retval, UA_STATUSCODE_GOOD);
    retval = UA_encodeBinaryWithOffset(obj1, &UA_TYPES[_i], &msg1, &pos, NULL, NULL);
    msg1.length = pos;
    ...
    ck_assert_int_eq(retval, UA_STATUSCODE_GOOD);
    pos = 0; retval = UA_encodeBinaryWithOffset(obj2, &UA_TYPES[_i], &msg2, &pos, NULL, NULL);
    msg2.length = pos;
    ck_assert_int_eq(retval, UA_STATUSCODE_GOOD);
...
START_TEST(calcSizeBinaryShallBeCorrect) {
    ...
    ck_assert_int_eq(retval, UA_STATUSCODE_GOOD);
    size_t offset = 0;
    retval = UA_encodeBinaryWithOffset(obj, &UA_TYPES[_i], &msg, &offset, NULL, NULL);
    if(retval)
        printf("%i\n",_i);
    ck_assert_int_eq(retval, UA_STATUSCODE_GOOD);
