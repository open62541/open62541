...
START_TEST(xml_bytestring_empty) {
    /* With the fix for COVERAGE_BUGS #8, UA_base64() returns NULL for
     * zero-length input instead of UA_EMPTY_ARRAY_SENTINEL. A non-NULL
     * data pointer with length=0 no longer crashes. */
    UA_Byte dummy = 0;
