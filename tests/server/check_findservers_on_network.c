...
START_TEST(find_on_network_large_cache_small_stack) {
    ...
    ck_assert_uint_eq(UA_ServerConfig_setDefault(UA_Server_getConfig(server)),
                      UA_STATUSCODE_GOOD);
    server->config.mdnsEnabled = true;

    for(UA_UInt32 i = 0; i < LARGE_DISCOVERY_CACHE_SIZE; i++) {
        serverOnNetwork_list_entry *entry = (serverOnNetwork_list_entry*)
            UA_calloc(1, sizeof(serverOnNetwork_list_entry));
        ck_assert_ptr_ne(entry, NULL);
        UA_ServerOnNetwork_init(&entry->serverOnNetwork);
        entry->serverOnNetwork.recordId = i;
        LIST_INSERT_HEAD(&server->discoveryManager.serverOnNetwork,
                         entry, pointers);
    }
    server->discoveryManager.serverOnNetworkRecordIdCounter =
        LARGE_DISCOVERY_CACHE_SIZE;
