 ...
 * invoked from the GDS driver start; the unit test applies it directly. */
UA_StatusCode initGDSRolePermissions(UA_Server *server);
...
START_TEST(GDS_SecurityAdmin_call_and_visibility) {
    ...
    /* The GDS build requires UA_NAMESPACE_ZERO=FULL, so the ServerConfiguration
     * object must exist. Fail loudly instead of silently skipping. */
    UA_QualifiedName bn;
    ck_assert_msg(UA_Server_readBrowseName(server, serverConfig, &bn) ==
                  UA_STATUSCODE_GOOD,
                  "ServerConfiguration (NS0 i=%u) must exist in a GDS-enabled build",
                  (unsigned)UA_NS0ID_SERVERCONFIGURATION);
    UA_QualifiedName_clear(&bn);

    /* Apply the GDS SecurityAdmin RolePermissions (driver start does this in a
     * running server; this unit test does not start the GDS driver). */
    ck_assert_uint_eq(initGDSRolePermissions(server), UA_STATUSCODE_GOOD);
