...
configure_lds_server(UA_Server *pServer) {
    ...
    UA_ServerConfig_setDefaultWithSecurityPolicies(
        config_lds, 4840, &certificate, &privateKey, NULL, 0, NULL, 0, NULL, 0);
...
setup_register(void) {
    ...
    *running_register = true;
    ...
    UA_ServerConfig_setDefaultWithSecurityPolicies(
        config_register, 16664, &certificate, &privateKey, NULL, 0, NULL, 0, NULL, 0);
...
registerServer(void) {
    ...
    *running_register = false;
    ...
    UA_StatusCode res = UA_Server_registerDiscovery(
        server_register, &cc, UA_STRING("opc.tcp://localhost:4840"), UA_STRING_NULL);
    *running_register = true;
    THREAD_CREATE(server_thread_register, serverloop_register);
...
unregisterServer(void) {
    ...
    *running_register = false;
    ...
    UA_StatusCode res = UA_Server_deregisterDiscovery(
...
Server_register_semaphore(void) {
    ...
    *running_register = false;
    ...
    UA_StatusCode res = UA_Server_registerDiscovery(server_register, &cc,
...
Server_unregister_semaphore(void) {
    // delete the semaphore, this should remove the registration automatically on next
    // check
...
FindAndCheck(const UA_String expectedUris[], size_t expectedUrisSize,
             const UA_String expectedLocales[], const UA_String expectedNames[],
    ...
    UA_StatusCode retval = UA_Client_findServers(
        client, "opc.tcp://localhost:4840", serverUrisSize, serverUris, localeIdsSize,
        localeIds, &applicationDescriptionArraySize, &applicationDescriptionArray);
...
FindOnNetworkAndCheck(UA_String expectedServerNames[], size_t expectedServerNamesSize,
    ...
    UA_StatusCode retval = UA_Client_findServersOnNetwork(
        client, "opc.tcp://localhost:4840", 0, 0, serverCapabilityFilterSize,
    ...
    if(serverOnNetwork != NULL) {
        for(size_t i = 0; i < expectedServerNamesSize; i++) {
            ...
            for(size_t j = 0;
                j < expectedServerNamesSize && !expectedServerNameInServerOnNetwork;
                j++) {
                expectedServerNameInServerOnNetwork = UA_String_equal(
                    &serverOnNetwork[j].serverName, &expectedServerNames[i]);
            }
            ck_assert_msg(expectedServerNameInServerOnNetwork,
                          "Expected %.*s in serverOnNetwork list, but not found",
                          (int)expectedServerNames[i].length,
                          expectedServerNames[i].data);
        }
    }
...
GetEndpoints(UA_Client *client, const UA_String *endpointUrl,
    ...
    request.endpointUrl =
        *endpointUrl;  // assume the endpointurl outlives the service call
    ...
    *endpointDescriptions = (UA_EndpointDescription *)UA_Array_new(
        response.endpointsSize, &UA_TYPES[UA_TYPES_ENDPOINTDESCRIPTION]);
...
// Test if discovery server lists himself as registered server if it is filtered by his
// uri
static UA_Boolean
...
Client_filter_locale(void) {
    ...
    // even if we request en-US, the server will return de-DE because it only has that
    // name.
...
Client_find_filter(void) {
    ...
    return FindAndCheck(expectedUris, 1, NULL, NULL, "urn:open62541.test.server_register",
                        NULL);
}
...
// Test if discovery server lists himself as registered server, before any other
// registration.
static UA_Boolean
