...
setup(void) {
    ...
    const UA_StatusCode retVal = loadNodesetFile(server,
...
START_TEST(Client_writeCustomEnum) {
    UA_UInt16 nsIdx = 0;
    ...
    UA_NodeId nodeId = UA_NODEID_NUMERIC(nsIdx, DEVICE_HEALTH_NODE_ID);
    const UA_StatusCode writeStatus =
        UA_Client_writeValueAttribute(client, nodeId, &value);
    ck_assert_uint_eq(writeStatus, UA_STATUSCODE_GOOD);

    UA_Variant stored;
    UA_Variant_init(&stored);
    ck_assert_uint_eq(UA_Server_readValue(server, nodeId, &stored),
                      UA_STATUSCODE_GOOD);
    const UA_NodeId dataTypeId = UA_NODEID_NUMERIC(nsIdx, 6244);
    ck_assert_ptr_ne(stored.type, NULL);
    ck_assert(UA_NodeId_equal(&stored.type->typeId, &dataTypeId));
    ck_assert_int_eq(*(UA_Int32*)stored.data, newValue);
...
testSuite_Client(void) {
    ...
    tcase_add_test(tc_client, Client_writeCustomEnum);
