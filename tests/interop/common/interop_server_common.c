 ...
 *
 * Shared OPC UA server for cross-SDK interoperability tests.
...
interopServerMain(int argc, char *argv[], UA_Boolean webSocket) {
    ...
    if(retval == UA_STATUSCODE_GOOD && webSocket) {
        ...
        retval = UA_ByteString_copy(&certificate,
                                    &config->webSocketCertificate);
        if(retval == UA_STATUSCODE_GOOD)
            retval = UA_ByteString_copy(&privateKey,
                                        &config->webSocketPrivateKey);
        if(retval == UA_STATUSCODE_GOOD) {
            ...
            snprintf(urlBuffer, sizeof(urlBuffer),
                     "opc.wss://:%u/interop", (unsigned)port);
