...
#ifdef UA_ENABLE_LWS
static UA_ByteString g_webSocketCaCertificate = {0, NULL};
#endif

static UA_StatusCode
setDefaultClientConfig(UA_ClientConfig *cc) {
    UA_StatusCode retval = UA_ClientConfig_setDefault(cc);
#ifdef UA_ENABLE_LWS
    if(retval == UA_STATUSCODE_GOOD && g_webSocketCaCertificate.length > 0)
        retval = UA_ByteString_copy(&g_webSocketCaCertificate,
                                    &cc->webSocketCaCertificate);
#endif
    return retval;
}
...
configureEncryption(UA_ClientConfig *cc, const char *policyUri,
    ...
    if(retval != UA_STATUSCODE_GOOD)
        return retval;
#ifdef UA_ENABLE_LWS
    if(g_webSocketCaCertificate.length > 0) {
        retval = UA_ByteString_copy(&g_webSocketCaCertificate,
                                    &cc->webSocketCaCertificate);
        if(retval != UA_STATUSCODE_GOOD)
            return retval;
...
int
interopClientMain(int argc, char *argv[], UA_Boolean webSocket) {
    if(argc < 2) {
        fprintf(stderr,
    ...
    const char *expectedPrefix = webSocket ? "opc.wss://" : "opc.tcp://";
    if(strncmp(url, expectedPrefix, strlen(expectedPrefix)) != 0) {
        fprintf(stderr, "Error: Expected a %s endpoint URL\n", expectedPrefix);
        return 1;
    }
...
#ifdef UA_ENABLE_LWS
    if(webSocket && trustListSize > 0)
        g_webSocketCaCertificate = trustList[0];
#else
    if(webSocket) {
