...
START_TEST(connectSubscribePublish) {
    ...
    UA_StatusCode res = mcm->openConnection(mcm, 3, params, NULL,
                                            &subscribeConnectionId, connectionCallback);
    ck_assert(res == UA_STATUSCODE_GOOD);

    uintptr_t publishConnectionId = 0;
    res = mcm->openConnection(mcm, 4, params, NULL, &publishConnectionId, connectionCallback);
    ck_assert(res == UA_STATUSCODE_GOOD);
    ...
    res = mcm->sendWithConnection(mcm, subscribeConnectionId, 0, NULL, &msg);
    ck_assert(res == UA_STATUSCODE_GOOD);

    /* Send with the publish connection */
    res = mcm->sendWithConnection(mcm, publishConnectionId, 0, NULL, &msg);
    ck_assert(res == UA_STATUSCODE_GOOD);
