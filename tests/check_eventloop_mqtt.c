...
START_TEST(connectSubscribePublish) {
    ...
    UA_KeyValueMap kvm2 = {4, params};
    uintptr_t publishConnectionId = 0;
    res = mcm->openConnection(mcm, &kvm2, NULL, &publishConnectionId, connectionCallback);
    ck_assert(res == UA_STATUSCODE_GOOD);
    ...
    res = mcm->sendWithConnection(mcm, subscribeConnectionId, &UA_KEYVALUEMAP_NULL, &msg);
    ck_assert(res == UA_STATUSCODE_GOOD);

    /* Send with the publish connection */
    res = mcm->sendWithConnection(mcm, publishConnectionId, &UA_KEYVALUEMAP_NULL, &msg);
    ck_assert(res == UA_STATUSCODE_GOOD);
