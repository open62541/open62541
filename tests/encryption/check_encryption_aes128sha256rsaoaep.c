...
int passwordCallbackCount = 0;

static UA_String passwordCallback(UA_PrivateKeyPasswordContextState state, UA_Boolean *doRetry,  void *context) {
    UA_String password;
    UA_String_init(&password);

    ++passwordCallbackCount;

    ck_assert(context == server);

    if (passwordCallbackCount == 1) {
        ck_assert(state == UA_PRIVATEKEYPASSWORDCONTEXTSTATE_INITIAL);
        password = UA_STRING_ALLOC("wrong");
        *doRetry = true;
    } else {
        ck_assert(state == UA_PRIVATEKEYPASSWORDCONTEXTSTATE_ERROR);
        password = UA_STRING_ALLOC("secret");
    }
...
START_TEST(encryption_connect_pem) {
    ...
    */
    ...
    UA_ClientConfig_setDefaultEncryption(cc, certificate, privateKey,
                                         trustList, trustListSize,
                                         revocationList, revocationListSize, passwordCallback, server);
    cc->securityPolicyUri =
        UA_STRING_ALLOC("http://opcfoundation.org/UA/SecurityPolicy#Aes128_Sha256_RsaOaep");
    ck_assert(client != NULL);
...
START_TEST(encryption_connect_pem_password) {
    UA_Client *client = NULL;
    UA_EndpointDescription* endpointArray = NULL;
    size_t endpointArraySize = 0;
    UA_ByteString *trustList = NULL;
    ...
    */
    ...
    UA_ClientConfig_setDefaultEncryption(cc, certificate, privateKey,
                                         trustList, trustListSize,
                                         revocationList, revocationListSize,
                                         passwordCallback, server);

    // Make sure the callback was called twice (one error, one successful attempt)
    ck_assert_int_eq(passwordCallbackCount, 2);
...
static Suite* testSuite_encryption(void) {
    ...
    tcase_add_test(tc_encryption, encryption_connect_pem_password);
