...
START_TEST(encryption_connect) {
    ...
    */
    ...
    cc->securityPolicyUri =
        UA_STRING_ALLOC("http://opcfoundation.org/UA/SecurityPolicy#ECC_nistP384");
    ck_assert(client != NULL);
...
START_TEST(encryption_connect_pem) {
    ...
    /* Load certificate and private key */
    UA_ByteString certificate;
    certificate.length = CERT_P384_PEM_LENGTH;
    certificate.data = CERT_P384_PEM_DATA;
    ck_assert_uint_ne(certificate.length, 0);

    UA_ByteString privateKey;
    privateKey.length = KEY_P384_PEM_LENGTH;
    privateKey.data = KEY_P384_PEM_DATA;
    ck_assert_uint_ne(privateKey.length, 0);
...
static Suite* testSuite_encryption(void) {
    ...
    TCase *tc_encryption = tcase_create("Encryption ECC_nistP384");
...
#ifdef __linux__ /* Linux only so far */
    TCase *tc_encryption_filestore = tcase_create("Encryption ECC_nistP384 security policy filestore");
