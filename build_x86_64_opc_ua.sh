...
cmake ${BUILD_OPTS} -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=${DEST_DIR_LOCAL} -LA ..
cmake --build .
sudo make install
...
cmake ${BUILD_OPTS} -DBUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=${DEST_DIR_LOCAL} -LA ..
cmake --build .
sudo make install