 ...
 *
 * NOTE: For now, received message processing is skipped i.e. address space is not
 * updated upon message reception. Instead a user callback function displays the received
 * message.
 *
 * **Connection handling**
...
#define MQTT_CLIENT_ID               "TESTCLIENTPUBSUBMQTT_SUB"
#define CONNECTIONOPTION_NAME        "mqttClientId"
#define TOPIC              			"customTopic"
...
void subscriber_message_callback(UA_ByteString *encodedBuffer, UA_ByteString *topic) {
	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "message received: topic=%s | size=%d.",
	        topic->data, (int)encodedBuffer->length);
	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "%s\n.", encodedBuffer->data);
}

static UA_StatusCode
addPubSubConnection(UA_Server *server, UA_NetworkAddressUrlDataType *networkAddressUrl) {
    ...
    /* Changed to static publisherId from tutorial_pubsub_mqtt_publish.c example*/
    ...
    if (UA_STATUSCODE_GOOD != retval)
    {
    	...
    	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "addPubSubConnection successful.");
    }
...
addDataSetReader(UA_Server *server, char *topic) {
     ...
     * tutorial_pubsub_publish_mqtt.c is being subscribed and is being updated in
    ...
    /* Double than publisher interval*/
    ...
    /* Subscribe to MQTT and register the callback function */
    retval |= UA_PubSubConnection_regist(server, &connectionIdentifier, &transportSettings, &subscriber_message_callback);
...
run(UA_String *transportProfile, UA_NetworkAddressUrlDataType *networkAddressUrl) {
    ...
    retval |= addPubSubConnection(server, networkAddressUrl);
    ...
    retval |= addDataSetReader(server, TOPIC);
    if (retval != UA_STATUSCODE_GOOD) {
    	UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "Error Name = %s", UA_StatusCode_name(retval));
    	return EXIT_FAILURE;
    }
