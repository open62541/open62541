 ...
 * Publishing Fields
 * ^^^^^^^^^^^^^^^^^
 * The PubSub mqtt publish subscribe example demonstrate the simplest way to publish and subscribe
...
#define MQTT_TOPIC            "customTopic"
#define METADATAQUEUENAME     "data"
#define METADATAUPDATETIME    0
...
addDataSetReader(UA_Server *server) {
    ...
    if(server == NULL) {
        ...
        addMqttSubscription(server, connection, MQTT_TOPIC, NULL, NULL, METADATAQUEUENAME, UA_BROKERTRANSPORTQUALITYOFSERVICE_BESTEFFORT);
...
addWriterGroup(UA_Server *server, int interval) {
    ...
    brokerTransportSettings.queueName = UA_STRING(MQTT_TOPIC);
...
addDataSetWriter(UA_Server *server) {
    ...
    brokerTransportSettings.metaDataUpdateTime = METADATAUPDATETIME;
...
int main(int argc, char **argv) {
    ...
    removeMqttSubscription(connection, MQTT_TOPIC, METADATAQUEUENAME);
    UA_Server_delete(server);
    return 0;
}
