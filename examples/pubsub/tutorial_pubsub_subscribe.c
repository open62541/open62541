...
addPubSubConnection (UA_Server *server, UA_String *transportProfile,
    ...
    UA_StatusCode retval = UA_Server_addPubSubConnection
...
addReaderGroup (UA_Server *server) {
    ...
    UA_StatusCode retval = UA_PubSubConnection_addReaderGroup
                           (server, connectionIdentifier, &readerGroupConfig,
...
addDataSetReader (UA_Server *server) {
    ...
    UA_StatusCode retval = UA_PubSubReaderGroup_addDataSetReader
                           (server, readerGroupIdentifier, &readerConfig,
                           &readerIdentifier);
    if (retval == UA_STATUSCODE_GOOD) {
        UA_LOG_INFO (UA_Log_Stdout, UA_LOGCATEGORY_SERVER,
...
static void addSubscribedVariables
            (UA_Server *server, UA_NodeId dataSetReaderId) {
    if (server == NULL) {
        return;
    }
    UA_NodeId folderId;
    ...
    UA_PubSubDataSetReader_addTargetVariables (server, &folderId,
                                               dataSetReaderId,
