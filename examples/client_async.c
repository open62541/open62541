...
static
void attrRead(UA_Client *client, void *userdata,
                                 UA_UInt32 requestId, void *response){
    reqNo--;
...
static
void fileBrowsed(UA_Client *client, void *userdata,
                                 UA_UInt32 requestId, void *response){
...
int main(int argc, char *argv[]) {
    ...
    retval = UA_Client_connect_async(client, "opc.tcp://localhost:4840");
    ...
    size_t reqId = 0;
    do{
    	/*limit reqId to avoid endless loop*/
    	if (reqId < 20){
    		UA_Client_addAsyncRequest(client,(void*)&bReq,&UA_TYPES[UA_TYPES_BROWSEREQUEST],fileBrowsed,&UA_TYPES[UA_TYPES_BROWSERESPONSE],NULL,&reqId);
    		reqNo++;
    ...
    UA_Variant_init(&val);
    for (int i = 0; i < 5; i++){
    	UA_Client_readValueAttribute_async(client, UA_NODEID_NUMERIC(0, UA_NS0ID_SERVER_SERVERSTATUS_CURRENTTIME), &val, attrRead, NULL);
