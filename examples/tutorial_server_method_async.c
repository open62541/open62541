...
helloWorldMethodCallback(UA_Server *server,
    ...
    UA_String *tmp = UA_String_new();
    *tmp = UA_STRING_ALLOC("Hello ");
...
static UA_NodeId
addHellWorldMethod(UA_Server *server, const UA_QualifiedName browseName) {
...
THREAD_CALLBACK(ThreadWorker) {
    UA_Server_runAsync(globalServer);
    return 0;
}
...
int main(void) {
    ...
    /* Add methods */
    UA_NodeId asyncMethodId =
        addHellWorldMethod(globalServer, UA_QUALIFIEDNAME(1, "AsyncHelloWorld"));
    UA_Server_setMethodNodeAsync(globalServer, asyncMethodId, true);

    UA_NodeId methodId =
	    addHellWorldMethod(globalServer, UA_QUALIFIEDNAME(1, "HelloWorld"));
    UA_Server_setMethodNodeAsync(globalServer, methodId, false); // just to be explicit

    UA_StatusCode retval = UA_Server_run(globalServer, &running);
    UA_Server_stopAsync(globalServer);
    /* Shutdown the thread */
    THREAD_JOIN(hThread);
