//
// Created by aribr on 02/02/2018.
//
...
/** Setting up an event
 * ^^^^^^^^^^^^^^^^^^^^^^
 * In order to set up the event, we can first use ``createEvent`` to give us a node representation of the event.
 ...
 * a node which emits the event - in this case the server node. We can use ``triggerEvent`` to trigger our
 * event onto said node. Note that once ``triggerEvent`` has been called, the node representation of the event is
 * deleted.
 */
 ...
 * require any input and as output we will be using the `EventId` we receive from ``triggerEvent``.
 *
