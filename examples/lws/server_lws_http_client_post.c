    ...
    if(params) {
        contentLength = UA_FORCE_CAST_PTR(UA_UInt64 *,
                                          UA_KeyValueMap_getScalar(params,
                                                                   UA_QUALIFIEDNAME(0, "contentLength"),
...
int main(int argc, const char **argv) {
    ...
    UA_String address = UA_STRING("localhost"); //httpbin.org
    UA_UInt16 port = 80; // 443 for https
    UA_Boolean useSSL = false; // true
    ...
    configParams[5].key = UA_QUALIFIEDNAME(0, "header");
    UA_Variant_setScalar(&configParams[5].value, &header, &UA_TYPES[UA_TYPES_STRING]);
    ...
    for(int i = 0; i < COUNT; i++) {
        ...
        cm->sendWithConnection(cm, requestId, &UA_KEYVALUEMAP_NULL, &msg);
    }
