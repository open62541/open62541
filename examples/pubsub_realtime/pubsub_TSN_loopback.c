 ...
 * This example has both Publisher and Subscriber(used as threads), the Subscriber thread subscribes to the counterdata published by the
 * Publisher thread of pubsub_TSN_publisher.c example. The subscribed counterdata is again published, which is subscribed by the Subscriber
 * thread of pubsub_TSN_publisher.c example. Thus a round-trip of counterdata is achieved.
 ...
 *
 * ./bin/examples/pubsub_TSN_loopback -i <iface>
 */
 ...
 *
 * ./bin/examples/pubsub_TSN_loopback <ethernet_interface> runs the application.
 *
 * For information, use ./bin/examples/pubsub_TSN_loopback -h.
 */
static void
usage(char *progname) {
