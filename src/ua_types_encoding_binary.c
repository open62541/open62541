...
UA_StatusCode UA_Double_decodeBinary(UA_ByteString const *src, size_t *offset,
	...
	//DBG_VERBOSE(
	//		printf("UA_Double_decodeBinary - biasedExponent52-55=%d, src=%d\n",
	//				biasedExponent, src->data[*offset + 6]));
	biasedExponent |= ((UA_UInt32)(src->data[*offset + 7] & 0x7F)) << 4;// bits 56-62
	//DBG_VERBOSE(
	//		printf("UA_Double_decodeBinary - biasedExponent56-62=%d, src=%d\n",
	//				biasedExponent, src->data[*offset + 7]));
...
static UA_StatusCode UA_NodeId_encodeBinary_nodeid_offset(UA_NodeId const *src,
	...
	case UA_NODEIDTYPE_NUMERIC:
		if (src->identifier.numeric + nodeid_offset > UA_UINT16_MAX
		...
		} else if (src->identifier.numeric + nodeid_offset > UA_BYTE_MAX
				|| src->namespaceIndex > 0) { /* UA_NODEIDTYPE_FOURBYTE */
...
UA_StatusCode UA_Variant_encodeBinary(UA_Variant const *src, UA_ByteString *dst,
	...
	for (UA_Int32 i = 0; i < numToEncode; i++) {
		...
		if (!isBuiltin) {
			...
			retval |= UA_NodeId_encodeBinary_nodeid_offset(&src->type->typeId,
					dst, offset,
...
UA_StatusCode UA_Variant_decodeBinary(UA_ByteString const *src,
	...
	if (isArray || typeIndex != UA_TYPES_EXTENSIONOBJECT) {
		...
		retval |= UA_Array_decodeBinary(src, offset,
				(!isArray && arraySize == -1) ? 1 : arraySize, &dst->data,
				dataType);
		if (retval != UA_STATUSCODE_GOOD)
			return retval;
		...
		UA_NodeId typeId;
		if ((retval |= UA_NodeId_decodeBinary(src, offset, &typeId))
				!= UA_STATUSCODE_GOOD)
			return retval;
		UA_Byte EOencodingByte;
		if ((retval |= UA_Byte_decodeBinary(src, offset, &EOencodingByte))
				!= UA_STATUSCODE_GOOD) {
		...
		if ((retval |= UA_decodeBinary(src, offset, dst->data, dataType))
				!= UA_STATUSCODE_GOOD)
			return retval;
		dst->type = dataType;
		dst->arrayLength = -1;
	...
	if (isArray && (encodingByte & UA_VARIANT_ENCODINGMASKTYPE_DIMENSIONS)) {
		...
		if (retval == UA_STATUSCODE_GOOD)
			retval |= UA_Array_decodeBinary(src, offset,
					dst->arrayDimensionsSize, &dst->data,
