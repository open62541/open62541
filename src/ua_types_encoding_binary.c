...
UA_Int32 UA_ExpandedNodeId_decodeBinary(UA_ByteString const *src, UA_UInt32 *offset,
    ...
    if(encodingByte & UA_EXPANDEDNODEID_NAMESPACEURI_FLAG) {
        ...
        CHECKED_DECODE(UA_String_decodeBinary(src, offset,
                                              &dst->namespaceUri), UA_ExpandedNodeId_deleteMembers(dst));
    }
    if(encodingByte & UA_EXPANDEDNODEID_SERVERINDEX_FLAG)
        CHECKED_DECODE(UA_UInt32_decodeBinary(src, offset,
                                              &dst->serverIndex), UA_ExpandedNodeId_deleteMembers(dst));
    return retval;
}
...
UA_Int32 UA_ExtensionObject_decodeBinary(UA_ByteString const *src, UA_UInt32 *offset,
    ...
    CHECKED_DECODE(UA_String_copy(&UA_STRING_NULL,
                                  (UA_String *)&dst->body), UA_ExtensionObject_deleteMembers(dst));
    switch(dst->encoding) {
    case UA_EXTENSIONOBJECT_ENCODINGMASK_NOBODYISENCODED:
        break;
    ...
    case UA_EXTENSIONOBJECT_ENCODINGMASK_BODYISXML:
        CHECKED_DECODE(UA_ByteString_decodeBinary(src, offset,
                                                  &dst->body), UA_ExtensionObject_deleteMembers(dst));
        break;
...
UA_Int32 UA_DataValue_decodeBinary(UA_ByteString const *src, UA_UInt32 *offset, UA_DataValue *dst) {
    ...
    if(dst->encodingMask & UA_DATAVALUE_ENCODINGMASK_SOURCETIMESTAMP)
        CHECKED_DECODE(UA_DateTime_decodeBinary(src, offset,
                                                &dst->sourceTimestamp), UA_DataValue_deleteMembers(dst));
    if(dst->encodingMask & UA_DATAVALUE_ENCODINGMASK_SOURCEPICOSECONDS) {
        CHECKED_DECODE(UA_Int16_decodeBinary(src, offset,
                                             &dst->sourcePicoseconds), UA_DataValue_deleteMembers(dst));
        if(dst->sourcePicoseconds > MAX_PICO_SECONDS)
            dst->sourcePicoseconds = MAX_PICO_SECONDS;
    }
    if(dst->encodingMask & UA_DATAVALUE_ENCODINGMASK_SERVERTIMESTAMP)
        CHECKED_DECODE(UA_DateTime_decodeBinary(src, offset,
                                                &dst->serverTimestamp), UA_DataValue_deleteMembers(dst));
    if(dst->encodingMask & UA_DATAVALUE_ENCODINGMASK_SERVERPICOSECONDS) {
        CHECKED_DECODE(UA_Int16_decodeBinary(src, offset,
                                             &dst->serverPicoseconds), UA_DataValue_deleteMembers(dst));
