...
UA_StatusCode UA_EndpointUrl_split(const char *endpointUrl, char *hostname, UA_UInt16 * port, char * path) {
    ...
    if (pathTmp) {
        ...
        strncpy(portStr, portTmp, strlen(portTmp));
		portStr[strlen(portTmp)]='\0';
		if (path) {
			path[0] = '\0';
		}
    }
