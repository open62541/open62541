...
static UA_StatusCode processDecodedOPNResponse(void *application,
	...
	UA_NodeId expectedId = UA_NODEID_NUMERIC(0,
			UA_TYPES[UA_TYPES_OPENSECURECHANNELRESPONSE].binaryEncodingId);
...
static UA_StatusCode processOPNResponse(void *application,
		UA_Connection *connection, UA_ByteString *chunk) {
	UA_Client *client = (UA_Client*) application;
...
UA_StatusCode UA_Client_connectInternal(UA_Client *client,
	...
	client->connection = client->config.connectionFunc(
...
UA_StatusCode UA_Client_connect_username(UA_Client *client,
...
static void sendCloseSession(UA_Client *client) {
	...
	__UA_Client_Service(client, &request,
			&UA_TYPES[UA_TYPES_CLOSESESSIONREQUEST], &response,
...
static void sendCloseSecureChannel(UA_Client *client) {
