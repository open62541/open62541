            ...
            for(size_t j = 0; j < dataChangeNotification->monitoredItemsSize; ++j) {
                ...
                LIST_FOREACH(mon, &sub->monitoredItems, listEntry) {
                    if(mon->clientHandle == mitemNot->clientHandle) {
                        mon->handler(sub->subscriptionID, mon->monitoredItemId, &mitemNot->value, mon->handlerContext);
                        break;
                    }
                }
                if(!mon)
            ...
            for (size_t j = 0; j < eventNotificationList->eventsSize; ++j) {
                ...
                LIST_FOREACH(mon, &sub->monitoredItems, listEntry) {
                    if(mon->clientHandle == eventFieldList->clientHandle) {
                        mon->handlerEvents(sub->subscriptionID, mon->monitoredItemId, eventFieldList->eventFieldsSize,
