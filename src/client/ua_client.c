...
static UA_StatusCode sendSymmetricServiceRequest(UA_Client *client,
...
static UA_StatusCode processAsyncResponse(UA_Client *client,
		UA_UInt32 requestId, UA_NodeId *responseTypeId,
		const UA_ByteString *responseMessage, size_t *offset, void *response) {
	/* Find the callback */
	AsyncServiceCall *ac;
 ...
 * SyncResponseDescription. */
static UA_StatusCode processServiceResponse(void *application,
	...
	const UA_NodeId serviceFaultNodeId = UA_NODEID_NUMERIC(0,
			UA_TYPES[UA_TYPES_SERVICEFAULT].binaryEncodingId);
	...
	if (!rd->responseType || requestId != rd->requestId) {
		retval = processAsyncResponse(rd->client, requestId, &responseId,
				message, &offset, rd->response);
		goto finish;
	}
	...
	if (UA_NodeId_equal(&responseId, &expectedNodeId)) {
		...
		retval = UA_decodeBinary(message, &offset, rd->response,
				rd->responseType, rd->client->config.customDataTypesSize,
	...
	finish: UA_NodeId_deleteMembers(&responseId);

	if (retval == UA_STATUSCODE_GOOD) {
		UA_LOG_DEBUG(rd->client->config.logger, UA_LOGCATEGORY_CLIENT,
				"Received a response of type %i",
...
static UA_StatusCode client_processChunk(void *application,
...
void __UA_Client_Service(UA_Client *client, const void *request,
...
UA_StatusCode UA_Client_runAsync(UA_Client *client, UA_UInt16 timeout) {
	/* TODO: Call repeated jobs that are scheduled */
	UA_DateTime maxDate = UA_DateTime_nowMonotonic()
			+ (timeout * UA_MSEC_TO_DATETIME);
	UA_StatusCode retval = receiveServiceResponse(client, NULL, NULL, maxDate,
	NULL);
