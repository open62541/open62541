...
processServiceResponse (void *application, UA_SecureChannel *channel,
    ...
    const UA_NodeId serviceFaultNodeId = UA_NODEID_NUMERIC (
            0, UA_TYPES[UA_TYPES_SERVICEFAULT].binaryEncodingId);
    ...
    if (UA_NodeId_equal (&responseId, &expectedNodeId)) {
        ...
        if (UA_NodeId_equal (&responseId, &serviceFaultNodeId)) {
            ...
            retval = UA_decodeBinary (message, &offset, rd->response,
                                      &UA_TYPES[UA_TYPES_SERVICEFAULT], 0,
                                      NULL);
        }
        else {
...
static UA_StatusCode
client_processChunk (void *application, UA_Connection *connection,
                     UA_ByteString *chunk) {
    SyncResponseDescription *rd = (SyncResponseDescription*) application;
