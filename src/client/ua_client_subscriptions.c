...
static void
UA_MonitoredItem_prepareModificationHandles(UA_Client *client, UA_Client_Subscription *sub,
                                                 UA_ModifyMonitoredItemsRequest *request) {
    for(size_t i = 0; i < request->itemsToModifySize; ++i) {
        ...
        if(mon && mon->isEventMonitoredItem) {
            ...
            request->itemsToModify[i].requestedParameters.clientHandle = mon->clientHandle;
        }
    }
}

static UA_Boolean
UA_MonitoredItem_hasDirtyEventMonitoredItem(UA_Client_Subscription *sub,
                                            const UA_ModifyMonitoredItemsRequest *request) {
    for(size_t i = 0; i < request->itemsToModifySize; ++i) {
        UA_Client_MonitoredItem *mon = findMonitoredItemById(sub, request->itemsToModify[i].monitoredItemId);

        if(mon && mon->isEventMonitoredItem && mon->isDirty)
            return true;
    }
...
static void
UA_MonitoredItem_resetDirtyEventMonitoredItems(UA_Client_Subscription *sub,
                                               const UA_ModifyMonitoredItemsRequest *request) {
    for(size_t i = 0; i < request->itemsToModifySize; ++i) {
        UA_Client_MonitoredItem *mon = findMonitoredItemById(sub, request->itemsToModify[i].monitoredItemId);
...
UA_Client_MonitoredItems_modify(UA_Client *client,
    ...
    if(UA_MonitoredItem_hasDirtyEventMonitoredItem(sub, &request)) {
        unlockClient(client);
        response.responseHeader.serviceResult = UA_STATUSCODE_BADINVALIDSTATE;
        return response;
    }

    UA_ModifyMonitoredItemsRequest modifiedRequest;
    UA_StatusCode copyResult = UA_ModifyMonitoredItemsRequest_copy(&request, &modifiedRequest);

    if(copyResult != UA_STATUSCODE_GOOD) {
        unlockClient(client);
        response.responseHeader.serviceResult = copyResult;
        return response;
    }

    UA_MonitoredItem_prepareModificationHandles(client, sub, &modifiedRequest);
...
UA_Client_MonitoredItems_modify_async(UA_Client *client,
    ...
    UA_MonitoredItem_prepareModificationHandles(client, sub, &data->request);
    ...
    if(statusCode != UA_STATUSCODE_GOOD) {
        UA_MonitoredItem_resetDirtyEventMonitoredItems(sub, &request);
        UA_ModifyMonitoredItemsRequest_clear(&data->request);
        UA_free(data);
    }
...
processEventNotification(UA_Client *client, UA_Client_Subscription *sub,
    ...
    for(size_t j = 0; j < eventNotificationList->eventsSize; ++j) {
        ...
        if(mon->isDirty) {
            UA_LOG_DEBUG(client->config.logging, UA_LOGCATEGORY_CLIENT,
                         "An event notification for monitored item %" PRIu32
                         " on subscription %" PRIu32
                         " was discarded because a modify operation is pending",
                         mon->monitoredItemId, sub->subscriptionId);
            continue;
        }
