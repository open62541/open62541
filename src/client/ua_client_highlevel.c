...
UA_StatusCode UA_Client_forEachChildNodeCall(UA_Client *client,
        UA_NodeId parentNodeId, UA_NodeIteratorCallback callback, void *handle) {
...
UA_StatusCode UA_Client_addReference(UA_Client *client,
        const UA_NodeId sourceNodeId, const UA_NodeId referenceTypeId,
    ...
    UA_AddReferencesResponse response = UA_Client_Service_addReferences(client,
            request);
...
UA_StatusCode UA_Client_deleteReference(UA_Client *client,
    ...
    UA_DeleteReferencesResponse response = UA_Client_Service_deleteReferences(
            client, request);
    UA_StatusCode retval = response.responseHeader.serviceResult;
    if (retval != UA_STATUSCODE_GOOD) {
...
UA_StatusCode UA_Client_deleteNode(UA_Client *client, const UA_NodeId nodeId,
    ...
    UA_DeleteNodesResponse response = UA_Client_Service_deleteNodes(client,
            request);
    UA_StatusCode retval = response.responseHeader.serviceResult;
    if (retval != UA_STATUSCODE_GOOD) {
...
UA_StatusCode __UA_Client_addNode(UA_Client *client,
        const UA_NodeClass nodeClass, const UA_NodeId requestedNewNodeId,
...
UA_StatusCode UA_Client_call(UA_Client *client, const UA_NodeId objectId,
...
UA_StatusCode UA_Client_writeArrayDimensionsAttribute(UA_Client *client,
    ...
    UA_Variant_setArray(&wValue.value.value,
            (void*) (uintptr_t) newArrayDimensions, newArrayDimensionsSize,
...
UA_StatusCode __UA_Client_readAttribute(UA_Client *client,
...
static UA_StatusCode processReadArrayDimensionsResult(UA_ReadResponse *response,
...
UA_StatusCode UA_Client_readArrayDimensionsAttribute(UA_Client *client,
    ...
    UA_ReadResponse response = UA_Client_Service_read(client, request);
    UA_StatusCode retval = processReadArrayDimensionsResult(&response,
            outArrayDimensions, outArrayDimensionsSize);
    UA_ReadResponse_deleteMembers(&response);
    return retval;
}
...
UA_StatusCode __UA_Client_readAttribute_async(UA_Client *client,
    ...
    request.nodesToRead = &item;
    request.nodesToReadSize = 1;
    outData outdata = { .attributeId = attributeId, .out = out, .outDataType =
            outDataType };

    return __UA_Client_AsyncService_withResponse(client, &request,
            &UA_TYPES[UA_TYPES_READREQUEST], callback, getResponseCallback,
 ...
 * Method Calling
 * ^^^^^^^^^^^^^^ */
UA_StatusCode __UA_Client_call_async(UA_Client *client, const UA_NodeId objectId,
        const UA_NodeId methodId, size_t inputSize, const UA_Variant *input,
