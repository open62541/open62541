...
updateCertificate(UA_Server *server,
    ...
    if(!server->transaction)
        server->transaction = UA_GDSTransaction_new(*sessionId);

    if(!UA_NodeId_equal(&server->transaction->sessionId, sessionId))
        return UA_STATUSCODE_BADTRANSACTIONPENDING;

    UA_StatusCode retval =
        UA_Server_updateCertificate(server, *certificateGroupId, *certificateTypeId,
...
openTrustList(UA_Server *server,
    ...
    if(!certGroup->fileInfoContext) {
        ...
        certGroup->fileInfoContext = (void*)fileInfo;
    }
    ...
    if(retval != UA_STATUSCODE_GOOD) {
        ...
        if(fileInfo->openCount == 0) {
            UA_free(fileInfo);
            certGroup->fileInfoContext = NULL;
        }
        return retval;
    }
...
closeAndUpdateTrustList(UA_Server *server,
               ...
               size_t inputSize, const UA_Variant *input,
               size_t outputSize, UA_Variant *output) {
    UA_StatusCode retval = UA_Server_applyChanges(server);
    if(retval != UA_STATUSCODE_GOOD)
        return retval;

    return closeTrustList(server, sessionId, sessionHandle, methodId, methodContext,
...
applyChanges(UA_Server *server,
    ...
    return UA_Server_applyChanges(server);
}
