...
writeOpenCountVariable(UA_Server *server, UA_CertificateGroup *group) {
    ...
    UA_FileInfo *fileInfo = (UA_FileInfo*)group->fileInfoContext;
...
updateCertificate(UA_Server *server,
    ...
    /* TODO: If there are issuer certificates, they must be attached to the certificate. */
...
addCertificate(UA_Server *server,
    ...
    UA_FileInfo *fileInfo = (UA_FileInfo*)certGroup->fileInfoContext;
    ...
    *changes = UA_GDSTRANSACTIONCHANGES_NOTHING;
    ...
    for(size_t i = 0; i < transaction->certGroupSize; i++) {
        *changes = UA_GDSTRANSACTIONCHANGES_TRUSTLIST;
        ...
        writeLastUpdateVariable(server, &server->config.sessionPKI);
    }
...
createFileInfoContexts(UA_Server *server) {
    ...
    UA_FileInfo *fileInfo = (UA_FileInfo*)UA_calloc(1, sizeof(UA_FileInfo));
    if(!fileInfo)
        return UA_STATUSCODE_BADOUTOFMEMORY;
    LIST_INIT(&fileInfo->fileContext);
    fileInfo->lastUpdateTime = lastUpdateTime;
    server->config.secureChannelPKI.fileInfoContext = (void*)fileInfo;

    UA_FileInfo *fileInfo2 = (UA_FileInfo*)UA_calloc(1, sizeof(UA_FileInfo));
    if(!fileInfo2) {
        UA_free(fileInfo);
        server->config.secureChannelPKI.fileInfoContext = NULL;
        return UA_STATUSCODE_BADOUTOFMEMORY;
    }
    LIST_INIT(&fileInfo2->fileContext);
    fileInfo2->lastUpdateTime = lastUpdateTime;
    server->config.sessionPKI.fileInfoContext = (void*)fileInfo2;
...
writeGroupVariables(UA_Server *server) {
    ...
    /* DefaultApplicationGroup */
    UA_FileInfo *fileInfoApplicationGroup = (UA_FileInfo*)server->config.secureChannelPKI.fileInfoContext;
    ...
    /* DefaultUserTokenGroup */
    UA_FileInfo *fileInfoUserTokenGroup = (UA_FileInfo*)server->config.sessionPKI.fileInfoContext;
