...
#define ADDPROFILEARRAY(x) profileArray[profileArraySize++] = UA_STRING_ALLOC(x)
    ...
    UA_VariableNode *enabledFlag = UA_VariableNode_new();
     ...
     enabledFlag->value.variantAndCallback.variant.data = UA_Boolean_new(); //initialized as false
      ...
      *stateEnum = UA_SERVERSTATE_RUNNING;
      ...
      state->value.variantAndCallback.variant.data = stateEnum; // points into the other object.
