...
typedef struct UA_AsyncOperation {
    ...
    union {
        ...
        struct {
            void *context;
            union {
                UA_ServerAsyncReadCallback read;
                UA_ServerAsyncWriteCallback write;
                UA_ServerAsyncMethodCallback call;
            } method;
        } callback;
    } handling;
...
write_async(UA_Server *server, UA_Session *session, const UA_WriteValue *operation,
            UA_ServerAsyncWriteCallback callback, void *context, UA_UInt32 timeout);

UA_StatusCode
call_async(UA_Server *server, UA_Session *session, const UA_CallMethodRequest *operation,
           UA_ServerAsyncMethodCallback callback, void *context, UA_UInt32 timeout);
