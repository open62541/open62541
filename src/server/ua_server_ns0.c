...
/**
 * Creates the basic nodes which are expected by the nodeset compiler to be already created.
 * This is necessary to reduce the dependencies for the nodeset compiler.
 */
static UA_StatusCode
 ...
 * This also initialized the data sources for various variables, such as for example server time.
 */
UA_StatusCode
UA_Server_initNS0(UA_Server *server) {
    ...
    /* NamespaceArray */
    UA_DataSource namespaceDataSource = {
            readNamespaces, // read
            NULL}; // write
...
#define ADDPROFILEARRAY(x) profileArray[profileArraySize++] = UA_STRING_ALLOC(x)
    ...
    /* ServiceLevel */
    UA_DataSource serviceLevel = {
            readServiceLevel, // read
            NULL}; // write
    retVal |= UA_Server_setVariableNode_dataSource(server,
                                                   UA_NODEID_NUMERIC(0, UA_NS0ID_SERVER_SERVICELEVEL), serviceLevel);

    /* Auditing */
    UA_DataSource auditing = {
            readAuditing, // read
            NULL}; // write
    retVal |= UA_Server_setVariableNode_dataSource(server,
