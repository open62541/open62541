...
void Service_GetEndpoints(UA_Server *server, UA_Session *session,
    ...
    size_t reSize = sizeof(UA_Boolean) * server->endpoints.count;
    UA_Boolean *relevant_endpoints = (UA_Boolean *)UA_alloca(reSize);
    memset(relevant_endpoints, 0, sizeof(UA_Boolean) * server->endpoints.count);
    size_t relevant_count = 0;
    if(request->profileUrisSize == 0) {
        for(size_t j = 0; j < server->endpoints.count; ++j)
            relevant_endpoints[j] = true;
        relevant_count = server->endpoints.count;
    } else {
        for(size_t j = 0; j < server->endpoints.count; ++j) {
            for(size_t i = 0; i < request->profileUrisSize; ++i) {
                if(!UA_String_equal(&request->profileUris[i],
                                    &server->endpoints.endpoints[j].endpointDescription.transportProfileUri))
                    continue;
                relevant_endpoints[j] = true;
                ++relevant_count;
                break;
    ...
    for(size_t i = 0; i < clone_times; ++i) {
        ...
        for(size_t j = 0; j < server->endpoints.count; ++j) {
            if(!relevant_endpoints[j])
                continue;
            retval |= UA_EndpointDescription_copy(&server->endpoints.endpoints[j].endpointDescription,
