...
static UA_StatusCode
getClientUserId(UA_Server *server, const UA_ExtensionObject *userIdentityToken,
    ...
    if(tokenType == UA_USERTOKENTYPE_CERTIFICATE) {
        ...
        if(server->config.sessionPKI.getSubjectName)
            return server->config.sessionPKI.getSubjectName(
                clientUserId, &userCertToken->certificateData);
    }
...
Service_ActivateSession(UA_Server *server, UA_SecureChannel *channel,
    ...
    /* The server does not support replacing the user identity of an active
     * Session. Reject the change before invoking access control so that the
     * plugin cannot mutate the existing Session context. Re-activation with
     * the same identity remains supported. */
    resp->responseHeader.serviceResult =
        getClientUserId(server, &req->userIdentityToken,
