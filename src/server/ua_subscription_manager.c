...
void Subscription_updateNotifications(UA_Subscription *subscription) {
	...
	(msg->notification->notificationData[notmsgn]).body.length = UA_Array_calcSizeBinary(changeNotification, monItemsChangeT, &UA_TYPES[UA_TYPES_DATACHANGENOTIFICATION]);
        ...
        // Deallocate the constructed ChangeNotification
        for(unsigned int i=0; i<monItemsChangeT; i++) {
          /* FIXME: It is unclear to me if the copyVariant()'s have allocated data... gicen the SEGFAULTS, I would speculate that no...
          if ((UA_MonitoredItemNotification *) &((changeNotification->monitoredItems)[i]).value.value.data != NULL) {
            UA_free((UA_MonitoredItemNotification *) &((changeNotification->monitoredItems)[i]).value.value.data);
          } */
          UA_free((UA_MonitoredItemNotification *) &((changeNotification->monitoredItems)[i]));
        }
