...
const UA_VariableNode *getArgumentsVariableNode(UA_Server *server, const UA_MethodNode *ofMethod, UA_String withBrowseName);
...
const UA_VariableNode *getArgumentsVariableNode(UA_Server *server, const UA_MethodNode *ofMethod, UA_String withBrowseName) {
    ...
    for(int i=0; i<ofMethod->referencesSize; i++) {
...
void Service_Call(UA_Server *server, UA_Session *session,
    ...
    for(int i=0; i<request->methodsToCallSize;i++){
        ...
        const UA_ObjectNode *withObject = (const UA_ObjectNode *) UA_NodeStore_get(server->nodestore, &rq->objectId);
