...
/* The method nodes has two properties for input and output arguments. They are variablenodes with a
   datasource that reads from the original methodnode. */

struct ReadMethodArgumentsHandle {
    const UA_NodeStore *ns;
    UA_NodeId methodNodeId;
};

static UA_StatusCode ReadMethodInputArguments(void *handle, UA_Boolean includeSourceTimeStamp,
                                              const UA_NumericRange *range, UA_DataValue *value) {
    const struct ReadMethodArgumentsHandle *args = (const struct ReadMethodArgumentsHandle*)handle;
    if(range) {
        value->hasStatus = UA_TRUE;
        value->status = UA_STATUSCODE_BADINDEXRANGEINVALID;
        return UA_STATUSCODE_GOOD;
    }
    const UA_MethodNode *node = (const UA_MethodNode*) UA_NodeStore_get(args->ns, &args->methodNodeId);
    if(!node)
        return UA_STATUSCODE_BADINTERNALERROR;
    UA_StatusCode retval = UA_Variant_setArrayCopy(&value->value, node->inputArguments,
                                                   node->inputArgumentsSize, &UA_TYPES[UA_TYPES_ARGUMENT]);
...
static UA_StatusCode ReadMethodOutputArguments(void *handle, UA_Boolean includeSourceTimeStamp,
                                               const UA_NumericRange *range, UA_DataValue *value) {
    const struct ReadMethodArgumentsHandle *args = (const struct ReadMethodArgumentsHandle*)handle;
    if(range) {
        value->hasStatus = UA_TRUE;
    ...
    if(!node)
        return UA_STATUSCODE_BADINTERNALERROR;
    UA_StatusCode retval = UA_Variant_setArrayCopy(&value->value, node->outputArguments,
                                                   node->outputArgumentsSize, &UA_TYPES[UA_TYPES_ARGUMENT]);
    if(retval == UA_STATUSCODE_GOOD) {
        value->hasValue = UA_TRUE;
        if(includeSourceTimeStamp) {
            value->hasSourceTimestamp = UA_TRUE;
...
/* Does nothing. We release the node already in the read callback of the datasource */
static void ReleaseMethodArguments(void *handle, UA_DataValue *value) {
    return;
}
...
UA_StatusCode UA_Server_addMethodNode(UA_Server *server, const UA_QualifiedName browseName, UA_NodeId nodeId,
    ...
    node->method = method;
    UA_StatusCode retval = UA_Array_copy(inputArguments, (void**)&node->inputArguments,
                                         &UA_TYPES[UA_TYPES_ARGUMENT], inputArgumentsSize);
    if(retval == UA_STATUSCODE_GOOD)
        node->inputArgumentsSize = inputArgumentsSize;
    retval = UA_Array_copy(outputArguments, (void**)&node->outputArguments, &UA_TYPES[UA_TYPES_ARGUMENT],
                           outputArgumentsSize);
    if(retval == UA_STATUSCODE_GOOD)
        node->outputArgumentsSize = outputArgumentsSize;
    retval = UA_NodeId_copy(&nodeId, &node->nodeId);
    if(retval != UA_STATUSCODE_GOOD) {
    ...
    UA_AddNodesResult res;
    if(retval == UA_STATUSCODE_GOOD) {
        res = UA_Server_addNode(server, (UA_Node*)node, parent, referenceTypeId);
    ...
    /* Add the input and output argument variables */
    struct ReadMethodArgumentsHandle *inputHandle = UA_malloc(sizeof(struct ReadMethodArgumentsHandle));
    if(inputHandle) {
        inputHandle->ns = server->nodestore;
        UA_NodeId_copy(&res.addedNodeId, &inputHandle->methodNodeId);
        UA_DataSource inputArgumentsSource = (UA_DataSource) {
            .handle = inputHandle, .read = ReadMethodInputArguments,
            .release = ReleaseMethodArguments, .write = UA_NULL};
        UA_Server_addDataSourceVariableNode(server, inputArgumentsSource,
                                            UA_QUALIFIEDNAME(0, "InputArguments"), UA_NODEID_NULL,
                                            res.addedNodeId, UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY));
    }

    struct ReadMethodArgumentsHandle *outputHandle = UA_malloc(sizeof(struct ReadMethodArgumentsHandle));
    if(outputHandle) {
        outputHandle->ns = server->nodestore;
        UA_NodeId_copy(&res.addedNodeId, &outputHandle->methodNodeId);
        UA_DataSource outputArgumentsSource = (UA_DataSource) {
            .handle = outputHandle, .read = ReadMethodInputArguments,
            .release = ReleaseMethodArguments, .write = UA_NULL};
        UA_Server_addDataSourceVariableNode(server, outputArgumentsSource,
                                            UA_QUALIFIEDNAME(0, "InputArguments"), UA_NODEID_NULL,
                                            res.addedNodeId, UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY));
    }
    return UA_STATUSCODE_GOOD;
...
UA_StatusCode UA_Server_AddMonodirectionalReference(UA_Server *server, UA_NodeId sourceNodeId,
