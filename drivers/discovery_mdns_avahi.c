...
typedef struct ServerOnNetwork {
    ...
    UA_Boolean srvSet;
    char* pathTmp;
    UA_UInt64 announceTimerId;
    UA_Boolean announced;
} ServerOnNetwork;
...
static void
announceTimerCallbackAvahi(void *application, void *context);

static void
multicastConflict(UA_String name, AvahiServerComponent *asc) {
    /* In case logging is disabled */
    (void)name;
    UA_LOG_ERROR(dm->drv.server->config.logging, UA_LOGCATEGORY_DISCOVERY,
...
addEntryToServersOnNetwork(AvahiServerComponent *asc, UA_String serverName,
    ...
    UA_LOG_DEBUG(dm->drv.server->config.logging, UA_LOGCATEGORY_SERVER,
    ...
    /* Do not announce immediately. Schedule a short timer that will forward
     * the entry to the server (or the serverAnnounced callback) after 1s if no
     * further details arrive. The caller (resolve/addRecord) will cancel the
     * timer and announce earlier if details become available. */
    listEntry->announceTimerId = 0;
    listEntry->announced = false;

    /* Add to the linked list */
    listEntry->next = asc->serverList;
    asc->serverList = listEntry;

    /* Schedule a one-shot announce timer that will forward the entry after 1s
     * if it wasn't announced already. Callers (resolve/addRecord) may cancel
     * this timer and announce earlier when they receive more details. */
    UA_Server *server = asc->msc.serverComponent.server;
    UA_EventLoop *el = UA_Server_getConfig(server)->eventLoop;
    UA_StatusCode tres = el->addTimer(el, (UA_Callback)announceTimerCallbackAvahi,
                                     asc, listEntry, (UA_Double)1000.0, NULL,
                                     UA_TIMERPOLICY_ONCE, &listEntry->announceTimerId);
    if(tres != UA_STATUSCODE_GOOD) {
        UA_LOG_WARNING(asc->logging, UA_LOGCATEGORY_DISCOVERY,
                       "Could not add announce timer for %S", serverName);
        listEntry->announceTimerId = 0;
    }
...
static void
announceTimerCallbackAvahi(void *application, void *context) {
    AvahiServerComponent *asc = (AvahiServerComponent*)application;
    ServerOnNetwork *entry = (ServerOnNetwork*)context;
    if(!asc || !entry)
        return;

    /* Check if entry is still present */
    ServerOnNetwork *cur = asc->serverList;
    while(cur && cur != entry) cur = cur->next;
    if(!cur)
        return;

    if(entry->announced) {
        entry->announceTimerId = 0;
        return;
    }

    UA_StatusCode res;
    if(asc->msc.serverAnnounced)
        res = asc->msc.serverAnnounced((UA_MdnsServerComponent*)&asc->msc,
                                        &entry->serverOnNetwork);
    else
        res = UA_Server_registerServerOnNetwork(asc->msc.serverComponent.server,
                                                &entry->serverOnNetwork, NULL);
    if(res != UA_STATUSCODE_GOOD) {
        UA_LOG_WARNING(asc->logging, UA_LOGCATEGORY_DISCOVERY,
                       "mDNS: announce callback failed for %S: %s",
...
entry_group_callback(AvahiEntryGroup *g, AvahiEntryGroupState state,
    ...
    switch(state) {
        case AVAHI_ENTRY_GROUP_ESTABLISHED:
            UA_LOG_INFO(dm->drv.server->config.logging, UA_LOGCATEGORY_DISCOVERY,
        ...
        case AVAHI_ENTRY_GROUP_FAILURE:
            UA_LOG_ERROR(
                dm->drv.server->config.logging, UA_LOGCATEGORY_DISCOVERY,
...
removeEntryFromServersOnNetwork(AvahiServerComponent *asc,
    ...
    if(entry) {
        ...
        /* Cancel any scheduled announce timer */
        if(entry->announceTimerId != 0) {
            UA_EventLoop *el = UA_Server_getConfig(asc->msc.serverComponent.server)->eventLoop;
            el->removeTimer(el, entry->announceTimerId);
            entry->announceTimerId = 0;
        }
...
addMdnsRecordForNetworkLayer(AvahiServerComponent *asc, const UA_String serverName,
    ...
    if(retval != UA_STATUSCODE_GOOD) {
        UA_LOG_WARNING(dm->drv.server->config.logging, UA_LOGCATEGORY_DISCOVERY,
    ...
    retval = addRecord(asc, serverName, hostname, port,
                       ...
                       config->mdnsSelfConfig.serverCapabilities,
                       config->mdnsSelfConfig.serverCapabilitiesSize, true);
    if(retval != UA_STATUSCODE_GOOD) {
        UA_LOG_WARNING(asc->logging, UA_LOGCATEGORY_DISCOVERY,
...
void client_callback(AvahiClient *c, AvahiClientState state, void *userdata) {
    ...
    UA_LOG_INFO(((UA_DiscoveryManager *)userdata)->drv.server->config.logging,
...
resolve_callback(AvahiServiceResolver *r, AVAHI_GCC_UNUSED AvahiIfIndex interface,
    ...
    switch(event) {
        ...
        case AVAHI_RESOLVER_FOUND: {
            ...
            /* Cancel any scheduled announce timer and announce immediately
             * because SRV provides the discoveryUrl. */
            if(listEntry->announceTimerId != 0) {
                UA_EventLoop *el = UA_Server_getConfig(asc->msc.serverComponent.server)->eventLoop;
                el->removeTimer(el, listEntry->announceTimerId);
                listEntry->announceTimerId = 0;
            }

            listEntry->srvSet = true;

            if(!listEntry->announced) {
                UA_StatusCode ares;
                if(asc->msc.serverAnnounced)
                    ares = asc->msc.serverAnnounced((UA_MdnsServerComponent*)&asc->msc,
                                                    &listEntry->serverOnNetwork);
                else
                    ares = UA_Server_registerServerOnNetwork(asc->msc.serverComponent.server,
                                                             &listEntry->serverOnNetwork, NULL);
                if(ares != UA_STATUSCODE_GOOD)
                    UA_LOG_WARNING(asc->logging, UA_LOGCATEGORY_DISCOVERY,
                                   "Could not store mDNS record for hostname %S",
                                   listEntry->serverOnNetwork.serverName);
                else
                    listEntry->announced = true;
            }

            break;
        }

        /* If SRV arrived, mark srvSet and announce immediately (cancel timer)
         * because SRV contains the discovery URL which is most important for
         * discovery. */
        default:
            UA_LOG_ERROR(asc->logging, UA_LOGCATEGORY_DISCOVERY,
...
addRecord(AvahiServerComponent *asc, const UA_String servername,
    ...
    if(retval != UA_STATUSCODE_BADALREADYEXISTS) {
        ...
        entry->srvSet = true;
         ...
         /* If a timer was scheduled during discovery, cancel it and announce now */
         if(entry->announceTimerId != 0) {
             UA_EventLoop *el = UA_Server_getConfig(asc->msc.serverComponent.server)->eventLoop;
             el->removeTimer(el, entry->announceTimerId);
             entry->announceTimerId = 0;
...
AvahiServerComponent_start(UA_ServerComponent *sc) {
    ...
    if(config->mdnsSelfAnnounce) {
        UA_String appName = config->mdnsSelfConfig.mdnsServerName;
        for(size_t i = 0; i < config->serverUrlsSize; i++)
            addMdnsRecordForNetworkLayer(asc, appName, &config->serverUrls[i]);
    }

    if(asc->msc.listen || asc->msc.queryPresence > 0) {
        asc->browser =
...
AvahiServerComponent_stop(UA_ServerComponent *sc) {
    ...
    if(config->mdnsSelfAnnounce) {
        for(size_t i = 0; i < config->serverUrlsSize; i++) {
            ...
            removeRecord(asc, config->mdnsSelfConfig.mdnsServerName, hostname, port);
        }
    }
...
AvahiServerComponent_free(UA_ServerComponent *sc) {
    ...
    while((son = asc->serverList)) {
        ...
        if(son->announceTimerId != 0) {
            el->removeTimer(el, son->announceTimerId);
            son->announceTimerId = 0;
        }
...
UA_MdnsServerComponent_Avahi(void) {
    ...
    msc->listen = true;
    msc->announceTTL = 600;
    msc->queryPresence = 60;
    msc->queryDetails = true;
    msc->respondToQueries = true;
