...
class open62541_MacroHelper():
  ...
  def getCreateNodeNoBootstrap(self, node, parentNode, parentReference, unprintedNodes):
    ...
    if node.nodeClass() == NODE_CLASS_METHOD:
        ...
        if len(inArgVal)>0:
            ...
            for inArg in inArgVal:
                ...
                argumentCnt += 1
            cleanupCode.append("UA_free(inputArguments);")
        if len(outArgVal)>0:
            code.append("")
            ...
            for outArg in outArgVal:
                ...
                argumentCnt += 1
            cleanupCode.append("UA_free(outputArguments);")
    ...
    if nodetype in ["Variable", "VariableType"]:
      code = code + node.printOpen62541CCode_SubtypeEarly(bootstrapping = False, cleanupCode=cleanupCode)
