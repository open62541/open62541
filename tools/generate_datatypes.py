...
static UA_INLINE UA_StatusCode %s_copy(const %s *src, %s *dst) { ''' + \
    ...
    def encoding_h(self, typeTableName):
        return '''static UA_INLINE UA_StatusCode %s_encodeBinary(const %s *src,UA_encodeBufferOverflowFcn overflowCallback,void *handle, UA_ByteString **dst, size_t *offset) { return UA_encodeBinary(src, %s,overflowCallback,handle,  dst, offset); }
