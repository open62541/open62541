...
""" % (outfilebase))
    ...
    writec(f"""
UA_StatusCode {outfilebase}_begin(UA_Server *server) {{
UA_StatusCode retVal = UA_STATUSCODE_GOOD;
""")

    # Generate namespaces (don't worry about duplicates)
    writec("/* Use namespace ids generated by the server */")
    writec(f"UA_UInt16 ns[{str(len(nodeset.namespaces))}];")
    for i, nsid in enumerate(nodeset.namespaces):
        nsid = nsid.replace("\"", "\\\"")
        writec(f'ns[{str(i)}] = UA_Server_addNamespace(server, "{nsid}");')
    ...
    for arr in set(typesArray):
        if arr == "UA_TYPES":
            continue
        writec(f"if({arr}_COUNT > 0)  {{")
        writec(f"custom{arr}.next = UA_Server_getConfig(server)->customDataTypes;")
        writec(f"UA_Server_getConfig(server)->customDataTypes = &custom{arr};\n")
        writec("}")

    if functionNumber > 0:
        for i in range(0, functionNumber):
            writec(f"if((retVal = function_{outfilebase}_{str(i)}_begin(server, ns)) != UA_STATUSCODE_GOOD) return retVal;")
            if i in reftypes_functionNumbers:
                writec(f"if((retVal = function_{outfilebase}_{str(i)}_finish(server, ns)) != UA_STATUSCODE_GOOD) return retVal;")
    writec("return retVal;\n}")

    writec(f"""
UA_StatusCode {outfilebase}_finish(UA_Server *server) {{
UA_StatusCode retVal = UA_STATUSCODE_GOOD;
""")
    # Generate namespaces (don't worry about duplicates)
    writec("/* Use namespace ids generated by the server */")
