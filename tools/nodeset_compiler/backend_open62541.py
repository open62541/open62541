...
""" % (outfilebase))
    ...
    for i in range(0, len(sorted_nodes)):
        writec("UA_StatusCode function" + str(i) + "(UA_Server *server, UA_UInt16 ns0);")
    
    for node in sorted_nodes:
        # Print node
        if not node.hidden:
            writec("\n/* " + str(node.displayName) + " - " + str(node.id) + " */")
            writec("\nUA_StatusCode function" + str(functionNumber) + "(UA_Server *server, UA_UInt16 ns0){\n")
...
UA_StatusCode retVal = UA_STATUSCODE_GOOD;""" % (outfilebase))
    ...
    for i, nsid in enumerate(nodeset.namespaces):
        ...
        for j in range(0, functionNumber):
            writec("retVal |= function" + str(j) + "(server, ns" + str(i) + ");")
