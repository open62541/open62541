...
#ifndef OPEN62541_CONNECTION_H
#define OPEN62541_CONNECTION_H
...
struct UA_Connection {
    ...
    /**
     * The chunk callback will be called once for each reassembled chunk.
     * The chunk needs to be copied by the callback if it needs to persist beyond the call.
     */
    ...
    UA_ByteString chunkBuffer;       /* A partially received chunk (TCP is a
...
/**
 * This function can be set as dataCallback in a socket.
 * It will assemble message chunks from the supplied data buffer.
 *
 * An incomplete chunk is stored in the connection until it is completed.
 * For each assembled chunk, the configured chunk callback is called.
 * \param buffer
 * \param sock
 * \return
 */
