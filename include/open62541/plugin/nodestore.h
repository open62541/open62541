...
#if !defined(UA_SERVER_H_) && !defined(OPEN62541_H_)
# error Include <open62541/server.h> and not this file directy
#endif
...
typedef struct {
    ...
    /* _newNode is used to create empty nodes of the different node types. The
     * memory is managed by the nodestore. Therefore, the node has to be removed
     * via a special deleteNode function. (If the new node is not added to the
     * nodestore.) */
    ...
    /* Release a node that has been retrieved with _getNode or
     * _getNodeFromPtr. */
