 ...
 * the VariableNode itself or external with callbacks to the application. */

/* If value is NULL, the Read service is used to get the latest value before
 * switching from an external to an internal value source. If notifications is
 * NULL, then all onRead/onWrite notifications are disabled. */
UA_StatusCode UA_EXPORT UA_THREADSAFE
UA_Server_setVariableNode_internalValueSource(
    UA_Server *server, const UA_NodeId nodeId, const UA_DataValue *value,
