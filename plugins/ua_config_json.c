...
static void
addSecurityPolicyToConfig(UA_Logger *logger, UA_ServerConfig *config, const UA_String policy, const UA_ByteString certificate, const UA_ByteString privateKey) {
#ifdef UA_ENABLE_ENCRYPTION
    ...
    if(UA_String_equal(&policy, &noneuri)) {
        ...
        if(retval != UA_STATUSCODE_GOOD) {
            UA_LOG_WARNING(logger, UA_LOGCATEGORY_APPLICATION,
        ...
        UA_LOG_WARNING(logger, UA_LOGCATEGORY_APPLICATION, "Unknown Security Policy.");
    }
    ...
    (void)privateKey;
    UA_LOG_WARNING(logger, UA_LOGCATEGORY_APPLICATION,
                   "Encryption support is not enabled. Cannot add security policy '%S'.",
                   policy);
#endif
}
...
PARSE_JSON(CertificateFileField) {
    ...
    if(retval == UA_STATUSCODE_GOOD) {
        if (filename.length > 0) {
            ...
            *certificate = loadCertificateFile((char const *)filename.data);
            UA_free(certfile);
#endif
    ...
    for(size_t j = tok.size; j > 0; j--) {
        ...
        if (retval == UA_STATUSCODE_GOOD) {
            if(certificate.length == 0 || privateKey.length == 0) {
                ...
                addSecurityPolicyToConfig(ctx->logging, config, policy, certificate, privateKey);
            }
        }
    ...
    for(size_t j = tok.size/2; j > 0; j--) {
        ...
        case CJ5_TOKEN_STRING: {
            ...
            if(strcmp(field_str, "endpointUrl") == 0)
                ...
                retval = ByteStringField_parseJson(ctx, &field->serverCertificate, NULL);
