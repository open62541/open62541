    ...
    for (size_t j = serverConfigSize; j > 0 && ctx.index < ctx.result.num_tokens; j--) {
        ...
        switch (tok.type) {
            case CJ5_TOKEN_STRING: {
                ...
                else if(strcmp(field, "allowUnattachedMethods") == 0)
                    retval = RuleHandlingField_parseJson(&ctx, &config->allowUnattachedMethods, NULL);
        ...
        switch (tok.type) {
            case CJ5_TOKEN_STRING: {
                ...
                if(strcmp(field, "timeout") == 0)
                    ...
                    retval = RuleHandlingField_parseJson(&ctx, &config->certificateEkuRule, NULL);
                else if(strcmp(field, "endpointDescriptionRule") == 0)
                    retval = RuleHandlingField_parseJson(
                        &ctx, &config->endpointDescriptionRule, NULL);
