 ...
 * Removed as soon as a patch is included in the main (https://github.com/warmcat/libwebsockets/pull/2924). */
...
sock_accept_custom(struct lws *wsi) {
    ...
    LWS_FD *conn = (LWS_FD*)ZIP_FIND(UA_FDTree, &priv->fds, &fd);
    ...
    ZIP_INSERT(UA_FDTree, &priv->fds, &newConn->rfd);
    priv->fdsSize++;
...
wsi_logical_close_custom(struct lws *wsi) {
    ...
    ZIP_REMOVE(UA_FDTree, &priv->fds, &conn->rfd);
    priv->fdsSize--;
    ...
    dc->next = priv->io_loop->delayedCallbacks;
    priv->io_loop->delayedCallbacks = dc;
