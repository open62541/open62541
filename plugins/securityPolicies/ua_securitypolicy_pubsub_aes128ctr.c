...
#define UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_SIGNING_KEY_LENGTH 16
#define UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_KEY_LENGTH 16
#define UA_SECURITYPOLICY_PUBSUB_AES128CTR_KEYNONCE_LENGTH 4
#define UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_ENCRYPTION_BLOCK_SIZE 16
#define UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_PLAIN_TEXT_BLOCK_SIZE 16
/* #define UA_SECURITYPOLICY_PUBSUB_AES128CTR_MINASYMKEYLENGTH 256
#define UA_SECURITYPOLICY_PUBSUB_AES128CTR_MAXASYMKEYLENGTH 512
counter block=keynonce(4Byte)+Messagenonce(8Byte)+counter(4Byte) see Part14 7.2.2.2.3.2
for details */
#define UA_SECURITYPOLICY_PUBSUB_AES128CTR_COUNTERBLOCK_SIZE 16
...
asym_verify_sp_pubsub_none_128ctr(const UA_SecurityPolicy *securityPolicy,
                           PUBSUB_AES128CTR_ChannelContext *cc,
...
static UA_StatusCode
asym_sign_sp_pubsub_none_128ctr(const UA_SecurityPolicy *securityPolicy,
                         PUBSUB_AES128CTR_ChannelContext *cc,
...
static UA_StatusCode
asym_encrypt_sp_pubsub_none_128ctr(const UA_SecurityPolicy *securityPolicy,
                            PUBSUB_AES128CTR_ChannelContext *cc, UA_ByteString *data) {
    return UA_STATUSCODE_GOOD;
}

static UA_StatusCode
asym_decrypt_sp_pubsub_none_128ctr(const UA_SecurityPolicy *securityPolicy,
                            PUBSUB_AES128CTR_ChannelContext *cc, UA_ByteString *data) {
    return UA_STATUSCODE_GOOD;
}
...
asym_getRemoteEncryptionKeyLength_sp_pubsub_length_none(
    const UA_SecurityPolicy *securityPolicy, const PUBSUB_AES128CTR_ChannelContext *cc) {
    return 0;
}
static size_t
asym_getRemoteBlockSize_sp_pubsub_length_none(const UA_SecurityPolicy *securityPolicy,
                                              const PUBSUB_AES128CTR_ChannelContext *cc) {
    return 0;
}
...
static UA_StatusCode
asym_makeThumbprint_sp_pubsub_none_128ctr(const UA_SecurityPolicy *securityPolicy,
                                   const UA_ByteString *certificate,
                                   UA_ByteString *thumbprint) {
    return UA_STATUSCODE_GOOD;
}
static size_t
asym_sp_pubsub_length_none_128ctr(const UA_SecurityPolicy *securityPolicy,
                           const PUBSUB_AES128CTR_ChannelContext *cc) {
    return 0;
}

static UA_StatusCode
asymmetricModule_compareCertificateThumbprint_sp_pubsub_none_128ctr(
...
static UA_StatusCode
sym_verify_sp_pubsub_aes128ctr(const UA_SecurityPolicy *securityPolicy,
                               PUBSUB_AES128CTR_ChannelContext *cc,
...
static UA_StatusCode
sym_sign_sp_pubsub_aes128ctr(const UA_SecurityPolicy *securityPolicy,
                             const PUBSUB_AES128CTR_ChannelContext *cc,
                             const UA_ByteString *message, UA_ByteString *signature) {
    if(signature->length != UA_SHA256_LENGTH)
        return UA_STATUSCODE_BADINTERNALERROR;
...
static size_t
sym_getSignatureSize_sp_pubsub_aes128ctr(const UA_SecurityPolicy *securityPolicy,
                                         const void *channelContext) {
    return UA_SHA256_LENGTH;
}

static size_t
sym_getSigningKeyLength_sp_pubsub_aes128ctr(const UA_SecurityPolicy *const securityPolicy,
                                            const void *const channelContext) {
    return UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_SIGNING_KEY_LENGTH;
}

static size_t
sym_getEncryptionKeyLength_sp_pubsub_aes128ctr(const UA_SecurityPolicy *securityPolicy,
                                               const void *channelContext) {
    return UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_KEY_LENGTH;
}

static size_t
sym_getEncryptionBlockSize_sp_pubsub_aes128ctr(
    const UA_SecurityPolicy *const securityPolicy, const void *const channelContext) {
    return UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_ENCRYPTION_BLOCK_SIZE;
}

static size_t
sym_getPlainTextBlockSize_sp_pubsub_aes128ctr(
    const UA_SecurityPolicy *const securityPolicy, const void *const channelContext) {
    return UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_PLAIN_TEXT_BLOCK_SIZE;
}

static UA_StatusCode
sym_encrypt_sp_pubsub_aes128ctr(const UA_SecurityPolicy *securityPolicy,
                                const PUBSUB_AES128CTR_ChannelContext *cc,
                                UA_ByteString *data) {
    ...
    UA_Byte
        counterBlockCopy[UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_ENCRYPTION_BLOCK_SIZE];
    UA_Byte aesBuffer[UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_ENCRYPTION_BLOCK_SIZE];
    memcpy(counterBlockCopy, cc->localSymIv.data,
           UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_ENCRYPTION_BLOCK_SIZE);
 ...
 * operations*/
static UA_StatusCode
sym_decrypt_sp_pubsub_aes128ctr(const UA_SecurityPolicy *securityPolicy,
                                const PUBSUB_AES128CTR_ChannelContext *cc,
                                UA_ByteString *data) {
    return sym_encrypt_sp_pubsub_aes128ctr(securityPolicy, cc, data);
}

/*Tested, meeting  Profile*/
static UA_StatusCode
sym_generateKey_sp_pubsub_aes128ctr(const UA_SecurityPolicy *securityPolicy,
...
/* This nonce does not need to be a
cryptographically random number, it can be pseudo-random */
static UA_StatusCode
sym_generateNonce_sp_pubsub_aes128ctr(const UA_SecurityPolicy *securityPolicy,
                                      UA_ByteString *out) {
...
static UA_StatusCode
parseRemoteCertificate_sp_pubsub_none_128ctr(PUBSUB_AES128CTR_ChannelContext *cc,
...
static UA_StatusCode
channelContext_newContext_sp_pubsub_aes128ctr(const UA_SecurityPolicy *securityPolicy,
                                              const UA_ByteString *remoteCertificate,
                                              void **pp_contextData) {
    ...
    *pp_contextData = UA_malloc(sizeof(PUBSUB_AES128CTR_ChannelContext));
    if(*pp_contextData == NULL)
        return UA_STATUSCODE_BADOUTOFMEMORY;

    PUBSUB_AES128CTR_ChannelContext *cc =
        (PUBSUB_AES128CTR_ChannelContext *)*pp_contextData;

    /* Initialize the channel context */
    cc->policyContext = (PUBSUB_AES128CTR_PolicyContext *)securityPolicy->policyContext;
    ...
    UA_StatusCode retval = parseRemoteCertificate_sp_pubsub_none_128ctr(cc, remoteCertificate);
    if(retval != UA_STATUSCODE_GOOD) {
        channelContext_deleteContext_sp_pubsub_aes128ctr(cc);
        *pp_contextData = NULL;
    }
    return retval;
}
...
static UA_StatusCode
channelContext_setLocalSymEncryptingKey_sp_pubsub_aes128ctr(
    PUBSUB_AES128CTR_ChannelContext *cc, const UA_ByteString *key) {
    if(key == NULL || cc == NULL)
        return UA_STATUSCODE_BADINTERNALERROR;
...
static UA_StatusCode
channelContext_setLocalSymSigningKey_sp_pubsub_aes128ctr(
    PUBSUB_AES128CTR_ChannelContext *cc, const UA_ByteString *key) {
    if(key == NULL || cc == NULL)
        return UA_STATUSCODE_BADINTERNALERROR;
...
static UA_StatusCode
channelContext_setLocalSymIv_sp_pubsub_aes128ctr(PUBSUB_AES128CTR_ChannelContext *cc,
...
static UA_StatusCode
channelContext_setRemoteSymEncryptingKey_sp_pubsub_aes128ctr(
    PUBSUB_AES128CTR_ChannelContext *cc, const UA_ByteString *key) {
    return UA_STATUSCODE_BADNOTIMPLEMENTED;
}

static UA_StatusCode
channelContext_setRemoteSymSigningKey_sp_pubsub_aes128ctr(
    PUBSUB_AES128CTR_ChannelContext *cc, const UA_ByteString *key) {
    return UA_STATUSCODE_BADNOTIMPLEMENTED;
}

static UA_StatusCode
channelContext_setRemoteSymIv_sp_pubsub_aes128ctr(PUBSUB_AES128CTR_ChannelContext *cc,
                                                  const UA_ByteString *iv) {
    return UA_STATUSCODE_BADNOTIMPLEMENTED;
}

static UA_StatusCode
channelContext_compareCertificate_sp_pubsub_none_128ctr(
    const PUBSUB_AES128CTR_ChannelContext *cc, const UA_ByteString *certificate) {

    return UA_STATUSCODE_GOOD;
}

static void
deleteMembers_sp_pubsub_aes128ctr(UA_SecurityPolicy *securityPolicy) {
...
static UA_StatusCode
updateCertificateAndPrivateKey_sp_pubsub_none_128ctr(UA_SecurityPolicy *securityPolicy,
...
static UA_StatusCode
policyContext_newContext_sp_pubsub_aes128ctr(UA_SecurityPolicy *securityPolicy,
                                             const UA_ByteString localPrivateKey) {
    UA_StatusCode retval = UA_STATUSCODE_GOOD;
    if(securityPolicy == NULL)
        return UA_STATUSCODE_BADINTERNALERROR;

    PUBSUB_AES128CTR_PolicyContext *pc = (PUBSUB_AES128CTR_PolicyContext *)UA_malloc(
        sizeof(PUBSUB_AES128CTR_PolicyContext));
...
UA_StatusCode
UA_SecurityPolicy_Pubsub_Aes128ctr(UA_SecurityPolicy *policy,
    ...
    asym_signatureAlgorithm->verify =
        (UA_StatusCode(*)(const UA_SecurityPolicy *, void *, const UA_ByteString *,
                          const UA_ByteString *))asym_verify_sp_pubsub_none_128ctr;
    asym_signatureAlgorithm->sign =
        (UA_StatusCode(*)(const UA_SecurityPolicy *, void *, const UA_ByteString *,
                          UA_ByteString *))asym_sign_sp_pubsub_none_128ctr;
    asym_signatureAlgorithm->getLocalSignatureSize =
        (size_t(*)(const UA_SecurityPolicy *, const void *))asym_sp_pubsub_length_none_128ctr;
    asym_signatureAlgorithm->getRemoteSignatureSize =
        (size_t(*)(const UA_SecurityPolicy *, const void *))asym_sp_pubsub_length_none_128ctr;
    asym_signatureAlgorithm->getLocalKeyLength =
        (size_t(*)(const UA_SecurityPolicy *,
                   const void *))asym_sp_pubsub_length_none_128ctr;  /* TODO: Write function */
    ...
    asym_encryptionAlgorithm->encrypt = (UA_StatusCode(*)(
        const UA_SecurityPolicy *, void *, UA_ByteString *))asym_encrypt_sp_pubsub_none_128ctr;
    asym_encryptionAlgorithm->decrypt = (UA_StatusCode(*)(
        const UA_SecurityPolicy *, void *, UA_ByteString *))asym_decrypt_sp_pubsub_none_128ctr;
    ...
    asym_encryptionAlgorithm->getRemotePlainTextBlockSize =
        (size_t(*)(const UA_SecurityPolicy *, const void *))asym_sp_pubsub_length_none_128ctr;
    asymmetricModule->makeCertificateThumbprint = asym_makeThumbprint_sp_pubsub_none_128ctr;
    asymmetricModule->compareCertificateThumbprint =
        asymmetricModule_compareCertificateThumbprint_sp_pubsub_none_128ctr;

    /* SymmetricModule */
    symmetricModule->generateKey = sym_generateKey_sp_pubsub_aes128ctr;
    symmetricModule->generateNonce = sym_generateNonce_sp_pubsub_aes128ctr;
    ...
    sym_signatureAlgorithm->verify =
        (UA_StatusCode(*)(const UA_SecurityPolicy *, void *, const UA_ByteString *,
                          const UA_ByteString *))sym_verify_sp_pubsub_aes128ctr;
    sym_signatureAlgorithm->sign =
        (UA_StatusCode(*)(const UA_SecurityPolicy *, void *, const UA_ByteString *,
                          UA_ByteString *))sym_sign_sp_pubsub_aes128ctr;
    sym_signatureAlgorithm->getLocalSignatureSize =
        sym_getSignatureSize_sp_pubsub_aes128ctr;
    sym_signatureAlgorithm->getRemoteSignatureSize =
        sym_getSignatureSize_sp_pubsub_aes128ctr;
    sym_signatureAlgorithm->getLocalKeyLength =
        (size_t(*)(const UA_SecurityPolicy *,
                   const void *))sym_getSigningKeyLength_sp_pubsub_aes128ctr;
    ...
    sym_encryptionAlgorithm->encrypt =
        (UA_StatusCode(*)(const UA_SecurityPolicy *, void *,
                          UA_ByteString *))sym_encrypt_sp_pubsub_aes128ctr;
    sym_encryptionAlgorithm->decrypt =
        (UA_StatusCode(*)(const UA_SecurityPolicy *, void *,
                          UA_ByteString *))sym_decrypt_sp_pubsub_aes128ctr;
    sym_encryptionAlgorithm->getLocalKeyLength =
        sym_getEncryptionKeyLength_sp_pubsub_aes128ctr;
    sym_encryptionAlgorithm->getRemoteKeyLength =
        sym_getEncryptionKeyLength_sp_pubsub_aes128ctr;
    sym_encryptionAlgorithm->getLocalBlockSize =
        (size_t(*)(const UA_SecurityPolicy *,
                   const void *))sym_getEncryptionBlockSize_sp_pubsub_aes128ctr;
    sym_encryptionAlgorithm->getRemoteBlockSize =
        (size_t(*)(const UA_SecurityPolicy *,
                   const void *))sym_getEncryptionBlockSize_sp_pubsub_aes128ctr;
    sym_encryptionAlgorithm->getLocalPlainTextBlockSize =
        (size_t(*)(const UA_SecurityPolicy *,
                   const void *))sym_getPlainTextBlockSize_sp_pubsub_aes128ctr;
    sym_encryptionAlgorithm->getRemotePlainTextBlockSize =
        (size_t(*)(const UA_SecurityPolicy *,
                   const void *))sym_getPlainTextBlockSize_sp_pubsub_aes128ctr;
    symmetricModule->secureChannelNonceLength =
        UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_SIGNING_KEY_LENGTH +
        UA_SECURITYPOLICY_PUBSUB_AES128CTR_SYM_KEY_LENGTH +
        UA_SECURITYPOLICY_PUBSUB_AES128CTR_KEYNONCE_LENGTH;
    ...
    /* ChannelModule */
    channelModule->newContext = channelContext_newContext_sp_pubsub_aes128ctr;
    channelModule->deleteContext =
        (void (*)(void *))channelContext_deleteContext_sp_pubsub_aes128ctr;
    ...
    channelModule->setLocalSymIv = (UA_StatusCode(*)(
        void *, const UA_ByteString *))channelContext_setLocalSymIv_sp_pubsub_aes128ctr;
    ...
    channelModule->setRemoteSymIv = (UA_StatusCode(*)(
        void *, const UA_ByteString *))channelContext_setRemoteSymIv_sp_pubsub_aes128ctr;
    ...
    policy->updateCertificateAndPrivateKey =
        updateCertificateAndPrivateKey_sp_pubsub_none_128ctr;

    policy->clear = deleteMembers_sp_pubsub_aes128ctr;

    return policyContext_newContext_sp_pubsub_aes128ctr(policy, localPrivateKey);
}
