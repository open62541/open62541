...
UA_skTrusted_Cert2X509(const UA_ByteString *certificateTrustList,
                       size_t certificateTrustListSize, CertContext *ctx) {
    size_t i;
    const unsigned char *pData;

    for(i = 0; i < certificateTrustListSize; i++) {
        pData = certificateTrustList[i].data;
        X509 *x509 = d2i_X509(NULL, &pData, (long)certificateTrustList[i].length);
        if(x509 == NULL) {
...
UA_skIssuer_Cert2X509(const UA_ByteString *certificateIssuerList,
                      size_t certificateIssuerListSize, CertContext *ctx) {
    size_t i;
    const unsigned char *pData;

    for(i = 0; i < certificateIssuerListSize; i++) {
        pData = certificateIssuerList[i].data;
        X509 *x509 = d2i_X509(NULL, &pData, (long)certificateIssuerList[i].length);
        if(x509 == NULL) {
            return UA_STATUSCODE_BADINTERNALERROR;
        }
...
UA_skCrls_Cert2X509(const UA_ByteString *certificateRevocationList,
                    size_t certificateRevocationListSize, CertContext *ctx) {
    size_t i;
    const unsigned char *pData;
    ...
    for(i = 0; i < certificateRevocationListSize; i++) {
        ...
        X509_CRL *crl =
            d2i_X509_CRL(NULL, &pData, (long)certificateRevocationList[i].length);
        if(crl == NULL) {
            return UA_STATUSCODE_BADINTERNALERROR;
        }
...
static int
UA_Certificate_Filter_der(const struct dirent *entry) {
    char *pszFind;
...
static int
UA_Certificate_Filter_crl(const struct dirent *entry) {
    char *pszFind;
...
static UA_StatusCode
UA_BuildFullPath(const char *path, const char *fileName, size_t fullPathBufferLength,
                 char *fullPath) {
...
static UA_StatusCode
UA_loadCertFromFile(const char *fileName, UA_ByteString *cert) {
...
UA_ReloadCertFromFolder(CertContext *ctx) {
    ...
    if(ctx->trustListFolder.length > 0) {
        ...
        (void)memcpy(folderPath, ctx->trustListFolder.data, ctx->trustListFolder.length);
        folderPath[ctx->trustListFolder.length] = 0;
        numCertificates =
            scandir(folderPath, &dirlist, UA_Certificate_Filter_der, alphasort);
        for(i = 0; i < numCertificates; i++) {
            if(UA_BuildFullPath(folderPath, dirlist[i]->d_name, PATH_MAX, certFile) !=
               UA_STATUSCODE_GOOD) {
                continue;
            }
    ...
    if(ctx->revocationListFolder.length > 0) {
        ...
        memcpy(folderPath, ctx->revocationListFolder.data,
               ctx->revocationListFolder.length);
        folderPath[ctx->revocationListFolder.length] = 0;
        numCertificates =
            scandir(folderPath, &dirlist, UA_Certificate_Filter_crl, alphasort);
        for(i = 0; i < numCertificates; i++) {
            if(UA_BuildFullPath(folderPath, dirlist[i]->d_name, PATH_MAX, certFile) !=
               UA_STATUSCODE_GOOD) {
                continue;
...
UA_CertificateVerification_Verify(void *verificationContext,
    ...
    if(X509_STORE_CTX_get_check_issued(storeCtx)(storeCtx, certificateX509,
                                                 certificateX509) != 1) {
        X509_STORE_CTX_set_flags(storeCtx,
...
UA_CertificateVerification_VerifyApplicationURI(void *verificationContext,
    ...
    ret = UA_STATUSCODE_BADSECURITYCHECKSFAILED;
    if(UA_Bstrstr(subjectURI.data, subjectURI.length, applicationURI->data,
                  applicationURI->length) != NULL) {
        ret = UA_STATUSCODE_GOOD;
    }
