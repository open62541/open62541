...
updateCertificateAndPrivateKey_sp_filestore(UA_SecurityPolicy *securityPolicy,
    ...
    UA_StatusCode retval =
        pc->policy.updateCertificateAndPrivateKey(securityPolicy, newCertificate, newPrivateKey);
    if(retval != UA_STATUSCODE_GOOD) {
        return retval;
    }
...
init_sp_filestore(UA_SecurityPolicy *policy) {
    ...
    policy->policyUri = pc->policy.policyUri;
    policy->securityLevel = pc->policy.securityLevel;
    policy->localCertificate = pc->policy.localCertificate;
    policy->certificateGroupId = pc->policy.certificateGroupId;
    policy->certificateTypeId = pc->policy.certificateTypeId;
    policy->asymmetricModule = pc->policy.asymmetricModule;
    policy->symmetricModule = pc->policy.symmetricModule;
    policy->certificateSigningAlgorithm = pc->policy.certificateSigningAlgorithm;
    policy->channelModule = pc->policy.channelModule;
    policy->logger = pc->policy.logger;
    policy->createSigningRequest = pc->policy.createSigningRequest;
...
UA_SecurityPolicy_Basic128Rsa15_Filestore(UA_SecurityPolicy *policy,
    ...
    retval = UA_SecurityPolicy_Basic128Rsa15(&pc->policy, localCertificate,
...
UA_SecurityPolicy_Basic256_Filestore(UA_SecurityPolicy *policy,
    ...
    retval = UA_SecurityPolicy_Basic256(&pc->policy, localCertificate,
                                        localPrivateKey, logger);
    if(retval != UA_STATUSCODE_GOOD)
        return retval;
...
UA_SecurityPolicy_Basic256Sha256_Filestore(UA_SecurityPolicy *policy,
    ...
    retval = UA_SecurityPolicy_Basic256Sha256(&pc->policy, localCertificate,
                                              localPrivateKey, logger);
    if(retval != UA_STATUSCODE_GOOD)
        return retval;
...
UA_SecurityPolicy_Aes128Sha256RsaOaep_Filestore(UA_SecurityPolicy *policy,
    ...
    retval = UA_SecurityPolicy_Aes128Sha256RsaOaep(&pc->policy, localCertificate,
                                                   localPrivateKey, logger);
    if(retval != UA_STATUSCODE_GOOD)
        return retval;
...
UA_SecurityPolicy_Aes256Sha256RsaPss_Filestore(UA_SecurityPolicy *policy,
    ...
    retval = UA_SecurityPolicy_Aes256Sha256RsaPss(&pc->policy, localCertificate,
                                                  localPrivateKey, logger);
    if(retval != UA_STATUSCODE_GOOD)
        return retval;
