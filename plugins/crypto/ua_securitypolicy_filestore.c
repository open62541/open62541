...
asymSig_getLocalKeyLength_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->asymSignatureAlgorithm.getLocalKeyLength(sp, channelContext);
}
...
asymSig_getRemoteKeyLength_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->asymSignatureAlgorithm.getRemoteKeyLength(sp, channelContext);
}
...
asym_encrypt_filestore(const UA_SecurityPolicy *sp, void *channelContext,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->asymEncryptionAlgorithm.encrypt(sp, channelContext, data);
}
...
asym_decrypt_filestore(const UA_SecurityPolicy *sp, void *channelContext,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->asymEncryptionAlgorithm.decrypt(sp, channelContext, data);
}
...
asym_getLocalKeyLength_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->asymEncryptionAlgorithm.getLocalKeyLength(sp, channelContext);
}
...
asym_getRemoteKeyLength_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->asymEncryptionAlgorithm.getRemoteKeyLength(sp, channelContext);
}
...
asym_getRemotePlainTextBlockSize_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->asymEncryptionAlgorithm.getRemotePlainTextBlockSize(sp, channelContext);
}
...
asym_getRemoteBlockSize_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->asymEncryptionAlgorithm.getRemoteBlockSize(sp, channelContext);
}
...
symSig_getLocalKeyLength_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->symSignatureAlgorithm.getLocalKeyLength(sp, channelContext);
}
...
symSig_getRemoteKeyLength_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->symSignatureAlgorithm.getRemoteKeyLength(sp, channelContext);
}
...
sym_encrypt_filestore(const UA_SecurityPolicy *sp, void *channelContext,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->symEncryptionAlgorithm.encrypt(sp, channelContext, data);
}
...
sym_decrypt_filestore(const UA_SecurityPolicy *sp, void *channelContext,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->symEncryptionAlgorithm.decrypt(sp, channelContext, data);
}
...
sym_getLocalKeyLength_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->symEncryptionAlgorithm.getLocalKeyLength(sp, channelContext);
}
...
sym_getRemoteKeyLength_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->symEncryptionAlgorithm.getRemoteKeyLength(sp, channelContext);
}
...
sym_getRemotePlainTextBlockSize_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->symEncryptionAlgorithm.getRemotePlainTextBlockSize(sp, channelContext);
}
...
sym_getRemoteBlockSize_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->symEncryptionAlgorithm.getRemoteBlockSize(sp, channelContext);
}
...
cert_getLocalKeyLength_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->certSignatureAlgorithm.getLocalKeyLength(sp, channelContext);
}
...
cert_getRemoteKeyLength_filestore(const UA_SecurityPolicy *sp,
    ...
    Filestore_Context *pc = (Filestore_Context *) sp->policyContext;
    UA_SecurityPolicy *isp = pc->innerPolicy;
    return isp->certSignatureAlgorithm.getRemoteKeyLength(sp, channelContext);
}
