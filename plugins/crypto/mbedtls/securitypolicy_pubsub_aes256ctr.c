...
verify_pubsub_aes256ctr(const UA_PubSubSecurityPolicy *policy,
    ...
    return UA_mbedTLS_PsaMacVerify(gc->signingKey.id,
...
newGroupContext_pubsub_aes256ctr(UA_PubSubSecurityPolicy *policy,
    ...
    if(signingKey) {
        UA_StatusCode res = UA_mbedTLS_PsaKey_import(&cc->signingKey,
            PSA_KEY_TYPE_HMAC, PSA_KEY_USAGE_SIGN_MESSAGE | PSA_KEY_USAGE_VERIFY_MESSAGE,
            PSA_ALG_HMAC(PSA_ALG_SHA_256), signingKey);
        if(res != UA_STATUSCODE_GOOD) {
            deleteGroupContext_pubsub_aes256ctr(policy, cc);
            return res;
        }
    }
    if(encryptingKey) {
        UA_StatusCode res = UA_mbedTLS_PsaKey_import(&cc->encryptingKey,
