...
static UA_StatusCode
asym_signAuth_sp_aes128sha256rsaoaep(ChannelContext_mbedtls *cc,
                                 const UA_ByteString *message,
                                 UA_ByteString *signature,
								 UA_ByteString *privateKey) {
    return channelContext_mbedtls_parseKeyThenSign(cc, message, signature, privateKey, mbedtls_sign_sha256);
}
...
UA_SecurityPolicy_Aes128Sha256RsaOaep(UA_SecurityPolicy *policy, const UA_Logger *logger) {
    ...
    asym_signatureAlgorithm->signAuth =
        (UA_StatusCode (*)(void *, const UA_ByteString *, UA_ByteString *, UA_ByteString*))asym_signAuth_sp_aes128sha256rsaoaep;
