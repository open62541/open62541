...
getSecurityKeys(UA_PubSubSecurityPolicy *sp,
    ...
    PUBSUB_AES256CTR_PolicyContext *pc =
        (PUBSUB_AES256CTR_PolicyContext*)sp->policyContext;
    ...
    rv = (UA_StatusCode)C_FindObjectsInit(pc->sessionHandle, attrTemplateSigningKey,
                                          sizeof(attrTemplateSigningKey)/sizeof (CK_ATTRIBUTE));
    if(rv == UA_STATUSCODE_GOOD) {
        signingKeyObjectFound = pkcs11_find_object_by_label(sp, pc->sessionHandle,
                                                            signingKeyLabel256_g,
                                                            &gc->signingKeyHandle);
...
static UA_StatusCode
newContext_pubsub_aes256ctr_tpm(UA_PubSubSecurityPolicy *policy,
    ...
    PUBSUB_AES256CTR_GroupContext *gc = (PUBSUB_AES256CTR_GroupContext *)
        UA_calloc(1, sizeof(PUBSUB_AES256CTR_GroupContext));
    if(gc == NULL)
    ...
    if(session)
        memcpy(&pc->sessionHandle, &session, sizeof(session));
...
static void
deleteContext_pubsub_aes256ctr_tpm(UA_PubSubSecurityPolicy *policy,
                                   void *gContext) {
    PUBSUB_AES256CTR_PolicyContext *pc =
        (PUBSUB_AES256CTR_PolicyContext*)policy->policyContext;
 ...
 * pseudo-random */
static UA_StatusCode
generateNonce_pubsub_aes256ctr_tpm(UA_PubSubSecurityPolicy *policy,
...
static UA_StatusCode
sign_pubsub_aes256ctr_tpm(const UA_PubSubSecurityPolicy *policy,
    ...
    UA_StatusCode rv = (UA_StatusCode)
        C_SignInit(pc->sessionHandle, &smech_256, gc->signingKeyHandle);
    if(rv != UA_STATUSCODE_GOOD) {
        UA_LOG_ERROR(policy->logger, UA_LOGCATEGORY_SECURITYPOLICY,
...
static UA_StatusCode
verify_pubsub_aes256ctr_tpm(const UA_PubSubSecurityPolicy *policy,
                            void *gContext, const UA_ByteString *message,
    ...
    UA_StatusCode rv = (UA_StatusCode)
        C_VerifyInit(pc->sessionHandle, &smech_256, gc->signingKeyHandle);
    if(rv != UA_STATUSCODE_GOOD) {
        UA_LOG_ERROR(policy->logger, UA_LOGCATEGORY_SECURITYPOLICY,
...
static UA_StatusCode
encrypt_pubsub_aes256ctr_tpm(const UA_PubSubSecurityPolicy *policy, void *gContext,
                             UA_ByteString *data) {
    ...
    memcpy(counterBlockEncrypt, &gc->keyNonceHandle, UA_AES256CTR_KEYNONCE_LENGTH);
    memcpy(counterBlockEncrypt + UA_AES256CTR_KEYNONCE_LENGTH,
           &gc->messageNonceHandle, UA_AES256CTR_MESSAGENONCE_LENGTH);
    ...
    rv = (UA_StatusCode)C_EncryptInit(pc->sessionHandle, &mech_256,
                                      gc->encryptingKeyHandle);
    if(rv != UA_STATUSCODE_GOOD) {
        UA_LOG_ERROR(policy->logger, UA_LOGCATEGORY_SECURITYPOLICY,
    ...
    while(rv == UA_STATUSCODE_GOOD &&
        ...
        rv = (UA_StatusCode)C_EncryptUpdate(pc->sessionHandle,
...
static UA_StatusCode
decrypt_pubsub_aes256ctr_tpm(const UA_PubSubSecurityPolicy *policy,
    ...
    memcpy(counterBlockDecrypt, &gc->keyNonceHandle, UA_AES256CTR_KEYNONCE_LENGTH);
    ...
    rv = (UA_StatusCode)C_DecryptInit(pc->sessionHandle, &mech_256,
                                      gc->encryptingKeyHandle);
    if(rv != UA_STATUSCODE_GOOD) {
        UA_LOG_ERROR(policy->logger, UA_LOGCATEGORY_SECURITYPOLICY,
                     "Decrypt init failed 0x%.8lX",
                     (long unsigned int)rv);
        return rv;
    }
    ...
    rv = (UA_StatusCode)C_DecryptFinal(pc->sessionHandle, &decodeFinal, &decodeFinalLen);
    if(rv != UA_STATUSCODE_GOOD) {
        UA_LOG_ERROR(policy->logger, UA_LOGCATEGORY_SECURITYPOLICY,
...
static UA_StatusCode
setMessageNonce_pubsub_aes256ctr_tpm(UA_PubSubSecurityPolicy *policy,
...
static size_t
getSignatureSize_pubsub_aes256ctr_tpm(const UA_PubSubSecurityPolicy *policy,
                                      const void *gContext) {
...
static size_t
getSignatureKeyLength_pubsub_aes256ctr_tpm(const UA_PubSubSecurityPolicy *policy,
                                           const void *gContext) {
    return UA_AES256CTR_SIGNING_KEY_LENGTH;
}

static size_t
getEncryptionKeyLength_pubsub_aes256ctr_tpm(const UA_PubSubSecurityPolicy *policy,
                                            const void *gContext) {
    return UA_AES256CTR_KEY_LENGTH;
}

static void
clear_pubsub_aes256ctr_tpm(UA_PubSubSecurityPolicy *policy) {
    UA_free(policy->policyContext);
    policy->policyContext = NULL;
}

static UA_StatusCode
setKeys_pubsub_aes256ctr_tpm(UA_PubSubSecurityPolicy *policy, void *gContext,
                             const UA_ByteString *signingKey,
                             const UA_ByteString *encryptingKey,
                             const UA_ByteString *keyNonce) {
    PUBSUB_AES256CTR_GroupContext *gc =
...
UA_StatusCode
UA_PubSubSecurityPolicy_Aes256CtrTPM(UA_PubSubSecurityPolicy *sp, char *userpin,
    ...
    sp->newGroupContext = newContext_pubsub_aes256ctr_tpm;
    sp->deleteGroupContext = deleteContext_pubsub_aes256ctr_tpm;
    sp->verify = verify_pubsub_aes256ctr_tpm;
    sp->sign = sign_pubsub_aes256ctr_tpm;
    sp->getSignatureSize = getSignatureSize_pubsub_aes256ctr_tpm;
    sp->getSignatureKeyLength = getSignatureKeyLength_pubsub_aes256ctr_tpm;
    sp->getEncryptionKeyLength = getEncryptionKeyLength_pubsub_aes256ctr_tpm;
    sp->encrypt = encrypt_pubsub_aes256ctr_tpm;
    sp->decrypt = decrypt_pubsub_aes256ctr_tpm;
    sp->setSecurityKeys = setKeys_pubsub_aes256ctr_tpm;
    sp->generateKey = NULL;
    sp->generateNonce = generateNonce_pubsub_aes256ctr_tpm;
    sp->keyMaterialLength = UA_AES256CTR_SIGNING_KEY_LENGTH +
