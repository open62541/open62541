...
UA_Policy_Basic128Rsa15_New_Context(
...
static UA_StatusCode
UA_ChannelModule_Basic128Rsa15_New_Context (
    ...
    context->policyContext = (Policy_Context_openssl *)
...
static size_t
asySig_Basic128Rsa15_getLocalSignatureSize(
	const Channel_Context_openssl* channelContext,
	EVP_PKEY* privateKey
) {
    if(channelContext == NULL)
...
static UA_StatusCode
UA_AsySig_Basic128Rsa15_Sign(void *channelContext, const UA_ByteString * message,
                              UA_ByteString *signature) {
    if(channelContext == NULL || message == NULL || signature == NULL)
        return UA_STATUSCODE_BADINTERNALERROR;
    ...
    return channelContext_loadKeyThenSign(
    	...
    	signature, asySig_Basic128Rsa15_Sign
	);
}
...
static size_t
asymEn_Basic128Rsa15_getLocalKeyLength (
	const Channel_Context_openssl* channelContext,
	EVP_PKEY* privateKey
) {
	if(channelContext == NULL)
...
asymEn_Basic128Rsa15_Decrypt(
    ...
    return UA_Openssl_RSA_PKCS1_V15_Decrypt(data, privateKey);
}

static UA_StatusCode
UA_AsymEn_Basic128Rsa15_Decrypt(void* channelContext, UA_ByteString* data) {
    if(channelContext == NULL || data == NULL)
        return UA_STATUSCODE_BADINVALIDARGUMENT;
   return channelContext_loadKeyThenDecrypt(
       (const Channel_Context_openssl*)channelContext, data, asymEn_Basic128Rsa15_Decrypt
   );
}
...
UA_SecurityPolicy_Basic128Rsa15 (UA_SecurityPolicy * policy,
    ...
    UA_StatusCode retval = UA_Policy_Basic128Rsa15_New_Context (policy, logger);
    if (retval != UA_STATUSCODE_GOOD) {
        return retval;
    }
