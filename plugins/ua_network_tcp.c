 ...
 * recv on the socket. */
static void connection_close(UA_Connection *connection) {
    shutdown((SOCKET) connection->sockfd, 2);
    connection->state = UA_CONNECTION_CLOSED;
}

static UA_StatusCode connection_getsendbuffer(UA_Connection *connection,
...
static void connection_releasesendbuffer(UA_Connection *connection,
        UA_ByteString *buf) {
    UA_ByteString_deleteMembers(buf);
}

static void connection_releaserecvbuffer(UA_Connection *connection,
        UA_ByteString *buf) {
    UA_ByteString_deleteMembers(buf);
}

static UA_StatusCode connection_write(UA_Connection *connection,
        UA_ByteString *buf) {
    ...
    do {
        ...
        do {
            ...
            n = send((SOCKET) connection->sockfd,
                    (const char*) buf->data + nWritten, WIN32_INT bytes_to_send,
                    flags);
...
static UA_StatusCode connection_recv(UA_Connection *connection,
    ...
    if (timeout > 0) {
        ...
        int resultsize = select(connection->sockfd + 1, &fdset, NULL, NULL,
                &tmptv);
...
static UA_StatusCode socket_set_blocking(SOCKET sockfd) {
#ifdef _WIN32
    u_long iMode = 0;
    if(ioctlsocket(sockfd, FIONBIO, &iMode) != NO_ERROR)
    return UA_STATUSCODE_BADINTERNALERROR;
...
typedef struct {
    ...
    LIST_HEAD(, ConnectionEntry)
    connections;
} ServerNetworkLayerTCP;

static void ServerNetworkLayerTCP_freeConnection(UA_Connection *connection) {
    UA_Connection_deleteMembers(connection);
    UA_free(connection);
}

static UA_StatusCode ServerNetworkLayerTCP_add(ServerNetworkLayerTCP *layer,
    ...
    int dummy = 1;
    if (setsockopt(newsockfd, IPPROTO_TCP, TCP_NODELAY, (const char *) &dummy,
            sizeof(dummy)) < 0) {
    ...
    int res = getnameinfo((struct sockaddr*) remote,
            sizeof(struct sockaddr_storage), remote_name, sizeof(remote_name),
    ...
    if (res == 0) {
        ...
        UA_LOG_WARNING(UA_Log_Stdout, UA_LOGCATEGORY_NETWORK,
                "Connection %i | New connection over TCP, "
                        "getnameinfo failed with errno %i", (int) newsockfd,
                errno__);
    }
...
static void addServerSocket(ServerNetworkLayerTCP *layer, struct addrinfo *ai) {
    ...
    if (setsockopt(newsock, SOL_SOCKET, SO_REUSEADDR, (const char *) &optval,
            sizeof(optval)) == -1) {
        UA_LOG_WARNING(UA_Log_Stdout, UA_LOGCATEGORY_NETWORK,
...
static UA_StatusCode ServerNetworkLayerTCP_start(UA_ServerNetworkLayer *nl) {
#ifdef _WIN32
...
static UA_Int32 setFDSet(ServerNetworkLayerTCP *layer, fd_set *fdset) {
    FD_ZERO(fdset);
...
static UA_StatusCode ServerNetworkLayerTCP_listen(UA_ServerNetworkLayer *nl,
        UA_Server *server, UA_UInt16 timeout) {
    ...
    {
        if (!UA_fd_isset(e->connection.sockfd,
                &errset) && !UA_fd_isset(e->connection.sockfd, &fdset))
            continue;

        UA_LOG_TRACE(UA_Log_Stdout, UA_LOGCATEGORY_NETWORK,
                "Connection %i | Activity on the socket", e->connection.sockfd);
...
static void ServerNetworkLayerTCP_stop(UA_ServerNetworkLayer *nl,
        UA_Server *server) {
    ServerNetworkLayerTCP *layer = (ServerNetworkLayerTCP *) nl->handle;
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_NETWORK,
...
UA_ServerNetworkLayer UA_ServerNetworkLayerTCP(UA_ConnectionConfig conf,
    ...
    ServerNetworkLayerTCP *layer = (ServerNetworkLayerTCP*) UA_calloc(1,
            sizeof(ServerNetworkLayerTCP));
    if (!layer)
        return nl;
...
UA_StatusCode UA_ClientConnectionTCPPoll(UA_Client *client, void *data) {
    ...
    if ((UA_Double) (UA_DateTime_nowMonotonic() - tcpConnection->connStart)
            * UA_DATETIME_TO_MSEC > tcpConnection->timeout) {
...
UA_Connection UA_ClientConnectionTCP(UA_ConnectionConfig conf,
        const char *endpointUrl, const UA_UInt32 timeout) {
#ifdef _WIN32
    ...
    do {
        ...
        /* Get a socket */
        clientsockfd = socket(server->ai_family, server->ai_socktype,
                server->ai_protocol);
#ifdef _WIN32
        if(clientsockfd == INVALID_SOCKET) {
#else
        ...
        if ((error == -1) && (errno__ != ERR_CONNECTION_PROGRESS)) {
            connection_close(&connection);
            UA_LOG_WARNING(UA_Log_Stdout, UA_LOGCATEGORY_NETWORK,
                    "Connection to %s failed with error: %s", endpointUrl,
                    strerror(errno__));
            freeaddrinfo(server);
            return connection;
        }
        ...
        if (error == -1 && (errno__ == ERR_CONNECTION_PROGRESS)) {
            ...
            UA_UInt32 timeSinceStart =
                    (UA_UInt32) ((UA_Double) (UA_DateTime_nowMonotonic()
                            - connStart) * UA_DATETIME_TO_MSEC);
            ...
            if (resultsize == 1) {
                ...
                int ret = getsockopt(clientsockfd, SOL_SOCKET, SO_ERROR,
                        &so_error, &len);
    ...
    } while ((UA_Double) (UA_DateTime_nowMonotonic() - connStart)
            * UA_DATETIME_TO_MSEC < timeout);
    ...
    if (!connected) {
        ...
        connection_close(&connection);
        UA_LOG_WARNING(UA_Log_Stdout, UA_LOGCATEGORY_NETWORK,
                "Trying to connect to %s timed out", endpointUrl);
        return connection;
    }
