...
UA_TCP_DataSocket_close(UA_Socket *sock) {
    ...
    sock->waitForReadActivity = false;
...
UA_TCP_DataSocket_recv(UA_Socket *socket, UA_ByteString *buffer, UA_UInt32 *timeout) {
    ...
    if(resultsize == 0) {
        UA_LOG_TRACE(socket->logger, UA_LOGCATEGORY_NETWORK, "Socket select timed out");
...
UA_TCP_DataSocket_init(UA_UInt64 sockFd,
    ...
    sock->socket.socketConfig.networkManager = networkManager;
    sock->socket.socketConfig.recvBufferSize = recvBufferSize;
    sock->socket.socketConfig.sendBufferSize = sendBufferSize;
...
UA_TCP_ClientDataSocket(void *application, const UA_ClientSocketConfig *parameters,
    ...
    sock->socket.socket.socketConfig.port = port;
