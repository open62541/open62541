...
tcp_sock_activity(UA_Socket *sock, UA_Boolean readActivity, UA_Boolean writeActivity) {
    if(readActivity) {
        ...
        socketConfig.networkManager = internalSock->socket.socketConfig.networkManager;
...
UA_TCP_ListenerSocketFromAddrinfo(struct addrinfo *addrinfo, const UA_ListenerSocketConfig *socketConfig,
    ...
    sock->socket.socketConfig = socketConfig->socketConfig;
    sock->socket.logger = sock->socket.socketConfig.networkManager->logger;
...
UA_TCP_ListenerSockets(void *application, const UA_ListenerSocketConfig *parameters,
    ...
    *discoveryUrlsSize = sockets_size;
    ...
    for(struct addrinfo *ai = res;
        ...
        ai = ai->ai_next, ++i) {
        UA_Socket *sock = NULL;
        retval = UA_TCP_ListenerSocketFromAddrinfo(ai, parameters,
