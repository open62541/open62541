...
wss_open(UA_Socket *sock) {
    if(sock->socketState != UA_SOCKETSTATE_NEW) {
        UA_LOG_ERROR(sock->socketConfig.networkManager->logger, UA_LOGCATEGORY_NETWORK,
...
wss_close(UA_Socket *socket) {
    UA_LOG_DEBUG(socket->socketConfig.networkManager->logger, UA_LOGCATEGORY_NETWORK,
    ...
    case LWS_CALLBACK_ADD_POLL_FD: {
        ...
        if(listenerSock->socket.socket.socketState == UA_SOCKETSTATE_OPEN &&
           listenerSock->socket.socket.id != socketFD) {
            ...
            socketConfig.networkManager = listenerSock->socket.socket.socketConfig.networkManager;

            UA_StatusCode retval = UA_WSS_DataSocket_AcceptFrom((UA_Socket *)listenerSock, &socketConfig,
                                                                &additionalParameters,
                                                                listenerSock->onAccept);
            if(retval != UA_STATUSCODE_GOOD) {
                UA_LOG_ERROR(listenerSock->socket.socket.socketConfig.networkManager->logger, UA_LOGCATEGORY_NETWORK,
...
UA_WSS_ListenerSocket(void *application, const UA_ListenerSocketConfig *parameters,
    ...
    sock->socket.socket.clean = wss_clean;
    ...
    sock->socket.socket.socketConfig = parameters->socketConfig;
    ...
    *discoveryUrlsSize = 1;
    ...
    if(!vhost) {
        UA_LOG_ERROR(sock->socket.socket.socketConfig.networkManager->logger, UA_LOGCATEGORY_NETWORK,
