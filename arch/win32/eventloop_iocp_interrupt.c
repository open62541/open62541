...
typedef struct UA_WIN32InterruptManager {
    ...
    UA_FD writefd;
    UA_atomic(struct UA_WIN32InterruptManager*)* managerSlot;
} UA_WIN32InterruptManager;
...
static UA_atomic(UA_WIN32InterruptManager*) interruptManagers[UA_MAX_INTERRUPT_MANAGERS];
...
static UA_Boolean
registerInterruptManager(UA_WIN32InterruptManager *pim) {
    for(size_t i = 0; i < UA_MAX_INTERRUPT_MANAGERS; i++) {
        UA_WIN32InterruptManager *prev = NULL;
...
static void
unregisterInterruptManager(UA_WIN32InterruptManager *pim) {
...
static void
closeSignalPipe(UA_WIN32InterruptManager *pim) {
    UA_EventLoopWIN32 *el = (UA_EventLoopWIN32*)pim->im.eventSource.eventLoop;
    (void)el;
    ...
    UA_EventLoopWIN32_deregisterFD(el, &pim->readfd);
...
static void
handleWIN32InterruptEvent(UA_EventSource *es, UA_RegisteredFD *rfd,
                          short event) {
    UA_EventLoopWIN32 *el = (UA_EventLoopWIN32 *)es->eventLoop;
    (void)el;
    (void)event;
    UA_LOCK_ASSERT(&el->elMutex);

    UA_WIN32InterruptManager *pim = (UA_WIN32InterruptManager*)es;
...
openSignalPipe(UA_WIN32InterruptManager *pim) {
    UA_EventLoopWIN32 *el = (UA_EventLoopWIN32*)pim->im.eventSource.eventLoop;
    (void)el;
    UA_LOCK_ASSERT(&el->elMutex);
    ...
    UA_FD fds[2];
    int err = UA_EventLoopWIN32_pipe(fds);
    if(err != 0) {
    ...
    pim->readfd.eventSourceCB = handleWIN32InterruptEvent;
    pim->readfd.listenEvents = UA_FDEVENT_IN;
    pim->readfd.fd = fds[0];
    pim->writefd = fds[1];

    UA_StatusCode res = UA_EventLoopWIN32_registerFD(el, &pim->readfd);
...
activateSignal(UA_RegisteredSignal *rs) {
    UA_EventLoopWIN32 *el = (UA_EventLoopWIN32*)rs->eventSource->eventLoop;
    (void)el;
    UA_LOCK_ASSERT(&el->elMutex);
    ...
    UA_WIN32InterruptManager *pim = (UA_WIN32InterruptManager*)rs->eventSource;
    UA_assert(pim->managerSlot != NULL);

    size_t managerSlot = (size_t)(pim->managerSlot - interruptManagers);

    /* Store the fd to be picked up by triggerWIN32InterruptEvent */
    rs->active = true;
    ...
    UA_RESET_ERRNO;
    void (*prev)(int) = signal(rs->signal, triggerWIN32InterruptEvent);
    if(prev != SIG_ERR) {
...
static UA_StatusCode
registerWIN32Interrupt(UA_InterruptManager *im, uintptr_t interruptHandle,
                       const UA_KeyValueMap *params,
                       UA_InterruptCallback callback, void *interruptContext) {
    UA_EventLoopWIN32 *el = (UA_EventLoopWIN32 *)im->eventSource.eventLoop;
    ...
    UA_WIN32InterruptManager *pim = (UA_WIN32InterruptManager *)im;
    UA_RegisteredSignal *rs;
    LIST_FOREACH(rs, &pim->signals, listPointers) {
        if(rs->signal == signal)
            break;
...
static void
deregisterWIN32Interrupt(UA_InterruptManager *im, uintptr_t interruptHandle) {
    UA_EventLoopWIN32 *el = (UA_EventLoopWIN32 *)im->eventSource.eventLoop;
    (void)el;
    UA_WIN32InterruptManager *pim = (UA_WIN32InterruptManager *)im;
    UA_LOCK(&el->elMutex);
...
UA_InterruptManager_new_WIN32(const UA_String eventSourceName) {
    UA_WIN32InterruptManager *pim = (UA_WIN32InterruptManager *)
        UA_calloc(1, sizeof(UA_WIN32InterruptManager));
    if(!pim)
        return NULL;
    ...
    im->registerInterrupt = registerWIN32Interrupt;
    im->deregisterInterrupt = deregisterWIN32Interrupt;
    return im;
}
...
UA_InterruptManager_new_POSIX(const UA_String eventSourceName) {
    return UA_InterruptManager_new_WIN32(eventSourceName);
}
