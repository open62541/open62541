...
static const struct {
    ...
    {{0, UA_STRING_STATIC("hostname")}, &UA_TYPES[UA_TYPES_STRING], true},
...
mqtt_pal_sendall(MQTTBrokerConnection *bc, const void* buf, size_t len, int flags) {
    ...
    res = tcpCM->sendWithConnection(tcpCM, bc->tcpConnectionId, 0, NULL, &msg);
    if(res != UA_STATUSCODE_GOOD)
        return MQTT_ERROR_SOCKET_ERROR;
...
removeTopicConnection(MQTTTopicConnection *tc) {
    ...
    tc->callback(&bc->mcm->cm, tc->topicConnectionId, tc->application, &tc->context,
                 UA_CONNECTIONSTATE_CLOSED, 2, kvp, UA_BYTESTRING_NULL);
...
removeBrokerConnection(MQTTBrokerConnection *bc) {
    ...
    UA_Array_delete((void*)bc->params, bc->paramsSize, &UA_TYPES[UA_TYPES_KEYVALUEPAIR]);
...
findIdenticalBrokerConnection(MQTTConnectionManager *mcm,
    ...
    for(MQTTBrokerConnection *bc = mcm->connections; bc; bc = bc->next) {
        ...
        for(size_t i = 0; i < MQTT_BROKERPARAMETERSSIZE; i++) {
            const UA_Variant *v1 =
                UA_KeyValueMap_get(bc->params, bc->paramsSize,
                                   MQTTConnectionParameters[i].name);
            const UA_Variant *v2 =
...
MQTTPublishResponseCallback(void** state, struct mqtt_response_publish *publish) {
    ...
    UA_String topic = {publish->topic_name_size,
        UA_FORCE_CAST_PTR(UA_Byte*, publish->topic_name)};
    UA_ByteString msg = {publish->application_message_size,
        UA_FORCE_CAST_PTR(UA_Byte*, publish->application_message)};
    ...
    for(MQTTTopicConnection *tc = bc->topicConnections; tc; tc = tc->next) {
        ...
        if(tc->topicConnectionState != UA_CONNECTIONSTATE_ESTABLISHED) {
            ...
            tc->callback(&bc->mcm->cm, tc->topicConnectionId,
                         ...
                         UA_CONNECTIONSTATE_ESTABLISHED, 2, kvp,
                         UA_BYTESTRING_NULL);
        }

        /* Forward the received message */
        tc->callback(&bc->mcm->cm, tc->topicConnectionId, tc->application,
                     &tc->context, UA_CONNECTIONSTATE_ESTABLISHED, 2, kvp, msg);
    }
}
        ...
        for(MQTTTopicConnection *tc = bc->topicConnections, *next = NULL;
            ...
            if(tc->subscribe) {
                ...
                tc->callback(&mcm->cm, tc->topicConnectionId, tc->application, &tc->context,
                             tc->topicConnectionState, 0, NULL, UA_BYTESTRING_NULL);
            }
        }
    }
...
createBrokerConnection(MQTTConnectionManager *mcm,
    ...
    /* Copy the parameters */
    UA_StatusCode res = UA_Array_copy(params, paramsSize, (void**)&bc->params,
                                      &UA_TYPES[UA_TYPES_KEYVALUEPAIR]);
    if(res != UA_STATUSCODE_GOOD) {
        UA_free(bc);
        return NULL;
    }
    bc->paramsSize = paramsSize;
    ...
    UA_UInt16 *keepAlive = UA_FORCE_CAST_PTR(UA_UInt16*,
        UA_KeyValueMap_getScalar(params, paramsSize,
    ...
    tcpParams[0].key = UA_QUALIFIEDNAME(0, "hostname");
    ...
    res = tcpCM->openConnection(tcpCM, 2, tcpParams, NULL, bc, MQTTNetworkCallback);
    if(res != UA_STATUSCODE_GOOD) {
        removeBrokerConnection(bc);
        return NULL;
    }
...
createTopicConnection(MQTTConnectionManager *mcm, MQTTBrokerConnection *bc,
    ...
    const UA_Boolean *_subscribe= (const UA_Boolean*)
        UA_KeyValueMap_getScalar(params, paramsSize, UA_QUALIFIEDNAME(0, "subscribe"),
                                 &UA_TYPES[UA_TYPES_BOOLEAN]);
    if(_subscribe)
        subscribe = *_subscribe;
    const UA_String *topic = (const UA_String*)
        UA_KeyValueMap_getScalar(params, paramsSize, UA_QUALIFIEDNAME(0, "topic"),
                                 &UA_TYPES[UA_TYPES_STRING]);
    if(topic->length == 0)
        return NULL;
...
static UA_StatusCode
MQTT_openConnection(UA_ConnectionManager *cm,
                   size_t paramsSize, const UA_KeyValuePair *params,
                   void *application, void *context,
                   UA_ConnectionManager_connectionCallback connectionCallback) {
    ...
    MQTTBrokerConnection *bc = findIdenticalBrokerConnection(mcm, paramsSize, params);
    if(!bc && !(bc = createBrokerConnection(mcm, paramsSize, params)))
        return UA_STATUSCODE_BADNOTCONNECTED;

    /* Create the per-topic connection */
    MQTTTopicConnection *tc =
        createTopicConnection(mcm, bc, paramsSize, params,
