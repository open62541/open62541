...
#define DEFAULT_HOSTNAME "lwip-default-host"
#endif
...
#else
    const char *defaultHostname = DEFAULT_HOSTNAME;
    size_t hostnameLen = strlen(defaultHostname);
    if(hostnameLen >= len)
        return -1;

    strncpy(name, defaultHostname, len);
#endif
    return 0;
}
